...
- subset data
- log-transform
- remove data with near-zero variance (caret package?)
- rerun plsda
- rerun perf and see what happens??

### Run initial PLS-DA

```{r}
X <- log((transMetabData %>% filter(., day != "0"))[c(14:198)]) #Assign known metabolite data to the X dataframe after log transformation
Y <- as.factor((transMetabData %>% filter(., day != "0"))$treatment_day) #Assign treatment data to the Y dataframe
```

```{r}
treatmentday.known.plsda <- plsda(X,Y, ncomp = 10, scale = TRUE) #Initially set a higher number for the number of components since we will be testing this
...
perf.treatmentday.known.plsda <- perf(treatmentday.known.plsda, validation = "Mfold", folds = 5,  
                     progressBar = TRUE, nrepeat = 50, BPPARAM = BiocParallel::SerialParam()) 

plot(perf.treatmentday.known.plsda, sd = TRUE) #Overall, increasing the number of components decreases the error rate. There doesn't seem to be an exponential reduction of error rate after 3 components.
```
...
### Rerun PLS-DA based on validated components
...
permanova.5GpresenceC #Significant influence of temperature, but not genotype or the interaction
```
...
permanova.5GpresenceT #Significant influence of temperature, but not genotype or the interaction
```
...
permanova.5GHet #Significant influence of temperature, but not genotype or the interaction
```
