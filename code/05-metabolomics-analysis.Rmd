...
```{r}
earlyvlate_5_pathwayResults <- read.csv("metaboanalyst/5_earlyvlate-metaboanalyst-results/pathway_results.csv") %>%
  dplyr::rename(metabolite = "X") %>%
  mutate(., Observed = Hits/Total) %>%
  mutate(., Significant = case_when(FDR < 0.05 ~ "Yes",
                                    FDR >= 0.05 ~ "No")) %>%
...
```{r}
earlyvlate_25_pathwayResults <- read.csv("metaboanalyst/25_earlyvlate-metaboanalyst-results/pathway_results.csv") %>%
  dplyr::rename(metabolite = "X") %>%
  mutate(., Observed = Hits/Total) %>%
  mutate(., Significant = case_when(FDR < 0.05 ~ "Yes",
                                    FDR >= 0.05 ~ "No")) %>%
