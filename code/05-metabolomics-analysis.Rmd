...
genotypeData <- read.csv("../../data/genotype/2023-crab-genotype.csv", header = TRUE, na.strings = "") %>%
  ...
  mutate(., presenceC = case_when(final.genotype == "CC" ~ "Y",
         ...
         presenceT = case_when(final.genotype == "CC" ~ "N",
                               final.genotype == "CT" ~ "Y",
                               final.genotype == "TT" ~ "Y")) #Import genotype data. Add additional informational columns
...
```{r}
transMetabData[,-c(1:11)] #All data
transMetabData[,c(12:196)] #Known data
```
...
```{r}
scaled.pca.T <- prcomp(transMetabData[-c(1:11)], scale = TRUE, center = TRUE) #Use prcomp to perform a centered and scaled PCA calculation
...
ordiMonteRandTest <- ordi.monte(transMetabData[-c(1:11)], ord = "pca", dim = 6, plot = FALSE) #Conduct monte carlo randomization test using 1000 random permutations of the input data.
ordiMonteRandTest
...
```{r}
transMetabData[c(12:196)] #All known metabolites
ncol(transMetabData[c(12:196)]) #There are 185 known metabolites
```

```{r}
scaled.pca.T.id <- prcomp(transMetabData[c(12:196)], scale = TRUE, center = TRUE) #Use prcomp to perform a centered and scaled PCA calculation
...
```{r}
dissim.T <- vegdist(scale(transMetabData[-c(1:11)]), "euclidean") #Create euclidean dissimilarity matrix to use for perMANOVA and PERMDISP
```
...
```{r}
dissim.T.id <- vegdist(scale(transMetabData[c(12:196)]), "euclidean") #Create euclidean dissimilarity matrix to use for perMANOVA and PERMDISP
```
...
```{r}
(transMetabData %>% filter(., day != "0"))[-c(1:11)] #Data set without control crabs (day 0)
```

```{r}
scaled.pca.TT <- prcomp((transMetabData %>% filter(., day != "0"))[-c(1:11)], scale = TRUE, center = TRUE) #Use prcomp to perform a centered and scaled PCA calculation
...
```{r}
(transMetabData %>% filter(., day != "0"))[c(12:196)] #Data set without control crabs (day 0)
```


```{r}
scaled.pca.TT.id <- prcomp((transMetabData %>% filter(., day != "0"))[c(12:196)], scale = TRUE, center = TRUE) #Use prcomp to perform a centered and scaled PCA calculation
...
```{r}
(transMetabData %>% filter(., day != "0"))[-c(1:11)]
```

```{r}
dissim.TT <- vegdist(scale((transMetabData %>% filter(., day != "0"))[-c(1:11)]), "euclidean") #Create euclidean dissimilarity matrix to use for perMANOVA and PERMDISP
```
...
```{r}
(transMetabData %>% filter(., day != "0"))[c(12:196)]
```

```{r}
dissim.TT.id <- vegdist(scale((transMetabData %>% filter(., day != "0"))[c(12:196)]), "euclidean") #Create euclidean dissimilarity matrix to use for perMANOVA and PERMDISP
```
...
```{r}
(transMetabData %>% filter(., day == "0" | day == "42"))[-c(1:11)] #Data from before and after chronic thermal stress
```

```{r}
scaled.pca.TG <- prcomp((transMetabData %>% filter(., day == "0" | day == "42"))[-c(1:11)], scale = TRUE, center = TRUE) #Use prcomp to perform a centered and scaled PCA calculation
...
```{r}
(transMetabData %>% filter(., day == "42"))[-c(1:11)] #Data from after chronic thermal stress
```

```{r}
scaled.pca.TGNC <- prcomp((transMetabData %>% filter(., day == "42"))[-c(1:11)], scale = TRUE, center = TRUE) #Use prcomp to perform a centered and scaled PCA calculation
...
```{r}
(transMetabData %>% filter(., day == "0" | day == "42"))[c(12:196)] #Data from before and after chronic thermal stress
```

```{r}
scaled.pca.TG.id <- prcomp((transMetabData %>% filter(., day == "0" | day == "42"))[c(12:196)], scale = TRUE, center = TRUE) #Use prcomp to perform a centered and scaled PCA calculation
...
```{r}
(transMetabData %>% filter(., day == "42"))[c(12:196)] #Data from after chronic thermal stress
```

```{r}
scaled.pca.TGNC.id <- prcomp((transMetabData %>% filter(., day == "42"))[c(12:196)], scale = TRUE, center = TRUE) #Use prcomp to perform a centered and scaled PCA calculation
...
```{r}
scaled.pca.TGNC.id.outlier <- prcomp((transMetabData %>% filter(., crab.ID != "5-075") %>% filter(., day == "42"))[c(12:196)], scale = TRUE, center = TRUE) #Use prcomp to perform a centered and scaled PCA calculation without the potential outlier
...
```{r}
dissim.TG <- vegdist(scale((transMetabData %>% filter(., day == "42"))[-c(1:11)]), "euclidean") #Create euclidean dissimilarity matrix to use for perMANOVA and PERMDISP
```
...
```{r}
dissim.TG.id <- vegdist(scale((transMetabData %>% filter(., day == "42"))[c(12:196)]), "euclidean") #Create euclidean dissimilarity matrix to use for perMANOVA and PERMDISP
```
...
```{r}
dissim.TG.id.outlier <- vegdist(scale((transMetabData %>% filter(., crab.ID != "5-075") %>% filter(., day == "42"))[c(12:196)]), "euclidean") #Create euclidean dissimilarity matrix to use for perMANOVA and PERMDISP
```
