...
```{r}
dat20230708clean <- dat20230708mod %>%
  filter(., perc_air_sat <= 100) %>% 
  group_by(tank, probe_num) %>%
  filter(., cumsum(perc_air_sat <= 80) == 0) %>%
  ungroup(.) %>%
...
```{r}
dat20230708cleanDOumol <- dat20230708mod %>%
  filter(., perc_air_sat <= 100) %>% 
  group_by(tank, probe_num) %>%
  filter(., cumsum(perc_air_sat <= 80) == 0) %>%
  ungroup(.) %>%
...
```{r}
dat20230708data <- dat20230708mod %>%
  filter(., perc_air_sat <= 100) %>% 
  group_by(tank, probe_num) %>%
  filter(., cumsum(perc_air_sat <= 80) == 0) %>%
  ungroup(.) %>%
...
head(dat20230708data) #Confirm object creation
```

```{r}
dat20230708_MO2_slopes <- dat20230708data %>%
...
dat20230708_MO2_slopes #Confirm dataframe creation
```
```{r}
dat20230708_MO2_glance <- dat20230708data %>%
  nest(data = -c(treatment, tank, probe_num)) %>%
  mutate(fit = map(data, ~lm(DO_umol_L ~ dt_s, data = .))) %>%
...
dat20230708_MO2_glance #Confirm dataframe creation
```

```{r}
dat20230708_MO2_slope_results <- dat20230708_MO2_slopes %>%
  filter(term == "dt_s") %>%
  mutate(slope_nmol_hr = estimate*1000*60) %>%
  dplyr::select(treatment, tank, probe_num, slope_nmol_hr, p.value) %>%
  mutate(r.squared = (dat20230708_MO2_glance$r.squared)) #Get the regression information for dt_s and not the intercept. Creates new a variable for respiration rate as nmol_per_hour (slope_nmol_hr). Select treatment, tank, probe number, slope over the hour, and p-value. Add the r-squared information from glance.
dat20230708_MO2_slope_results #Confirm dataframe creation
```

```{r}
dat20230708_MO2_slope_results %>%
...
```{r}
dat20230708blanks <- dat20230708mod %>%
  filter(., !(tank == "tank1" & dt_s < 3505)) %>%
  filter(., !(tank == "tank2" & dt_s < 4480)) %>%
  filter(., !(tank == "tank3" & probe_num == 1 & dt_s < 840)) %>%
  filter(., !(tank == "tank3" & probe_num == 2 & dt_s < 710)) %>%
  ...
  mutate(., dt_s = ifelse(tank == "tank5" & probe_num == 3, dt_s - 7920, dt_s)) %>%
  mutate(., dt_s = ifelse(tank == "tank6", dt_s - 882, dt_s))
head(dat20230708blanks) #Confirm dataframe creation
```

```{r}
dat20230708_MO2_slopes_blanks <- dat20230708blanks %>%
  nest(data = -c(treatment, tank, probe_num)) %>%
  mutate(fit = map(data, ~lm(DO_umol_L ~ dt_s, data = .))) %>%
  mutate(tidied = map(fit, tidy)) %>%
  unnest(tidied) #Create a list object that stores the regression results for each chamber. Nest data by treatment, tank, and probe number so the regression slopes are calculated appropriately. Use map to extract linear model fit information for DO_umol_L by time. Extract slopes using broom::tidy and unnest.
dat20230708_MO2_slopes_blanks #Confirm dataframe creation
```

```{r}
dat20230708_MO2_glance_blanks <- dat20230708blanks %>%
  nest(data = -c(treatment, tank, probe_num)) %>%
  mutate(fit = map(data, ~lm(DO_umol_L ~ dt_s, data = .))) %>%
  mutate(glanced = map(fit, glance)) %>%
  unnest(glanced) #Similar to above, but calculate the adjusted R-squared values by nesting treatment and probe information
dat20230708_MO2_glance_blanks #Confirm dataframe creation
```

```{r}
dat20230708_MO2_slopes_blanks_results <- dat20230708_MO2_slopes_blanks %>%
  filter(term == "dt_s") %>%
  mutate(slope_nmol_hr = estimate*1000*60) %>%
  dplyr::select(treatment, tank, probe_num, slope_nmol_hr, p.value) %>%
  mutate(r.squared = (dat20230708_MO2_glance_blanks$r.squared)) #Get the regression information for dt_s and not the intercept. Creates new a variable for respiration rate as nmol_per_hour (slope_nmol_hr). Select treatment, tank, probe number, slope over the hour, and p-value. Add the r-squared information from glance.
dat20230708_MO2_slopes_blanks_results #Confirm dataframe creation
```

```{r}
dat20230708_MO2_all_slope_results <- dat20230708_MO2_slope_results %>%
  left_join(., dat20230708_MO2_slopes_blanks_results, by = c("treatment", "tank", "probe_num"), suffix = c("", "_blank")) %>%
  mutate(., slope_nmol_hr_corrected = slope_nmol_hr - dat20230708_MO2_slopes_blanks_results$slope_nmol_hr) %>%
...
dat20230708_MO2_all_slope_results
```

```{r}
write.csv(dat20230708_MO2_all_slope_results, "20220708-MO2-slope-results.csv", quote = FALSE, row.names = FALSE) #Save results with uncorrected, blank, and corrected slopes
```
...
```{r}
hist(-(dat20230708_MO2_slope_results$slope_nmol_hr_corrected)) #Check distribution
hist(log(-(dat20230708_MO2_slope_results$slope_nmol_hr_corrected))) #Check log transformation
hist(sqrt(-(dat20230708_MO2_slope_results$slope_nmol_hr_corrected))) #Check sqrt transformation
```

```{r}
lm20220708 <- lm(log(-slope_nmol_hr_corrected) ~ treatment, data = dat20230708_MO2_slope_results)
summary(lm20220708)
```
...
```{r}
hist(-(dat20230712_MO2_slope_results$slope_nmol_hr_corrected)) #Check distribution
hist(log(-(dat20230712_MO2_slope_results$slope_nmol_hr_corrected))) #Check log transformation
hist(sqrt(-(dat20230712_MO2_slope_results$slope_nmol_hr_corrected))) #Check sqrt transformation
```

```{r}
lm20220712 <- lm(log(-slope_nmol_hr_corrected) ~ treatment, data = dat20230712_MO2_slope_results)
summary(lm20220712)
```
...
```{r}
hist(-(dat20230719_MO2_slope_results$slope_nmol_hr_corrected)) #Check distribution
hist(log(-(dat20230719_MO2_slope_results$slope_nmol_hr_corrected))) #Check log transformation
hist(sqrt(-(dat20230719_MO2_slope_results$slope_nmol_hr_corrected))) #Check sqrt transformation
```

```{r}
lm20220719 <- lm(log(-slope_nmol_hr_corrected) ~ treatment, data = dat20230719_MO2_slope_results)
summary(lm20220719)
```
...
```{r}
hist(-(dat20230726_MO2_slope_results$slope_nmol_hr_corrected)) #Check distribution
hist(log(-(dat20230726_MO2_slope_results$slope_nmol_hr_corrected))) #Check log transformation
hist(sqrt(-(dat20230726_MO2_slope_results$slope_nmol_hr_corrected))) #Check sqrt transformation
```

```{r}
lm20220726 <- lm(log(-slope_nmol_hr_corrected) ~ treatment, data = dat20230726_MO2_slope_results)
summary(lm20220726)
```
...
```{r}
all_MO2_slope_results <- bind_rows("04" = dat20230708_MO2_all_slope_results,
                                   "08" = dat20230712_MO2_all_slope_results,
                                   "15" = dat20230719_MO2_all_slope_results,
                                   "22" = dat20230726_MO2_all_slope_results, 
