...
> To answer this question, I would compare 15ºC vs. 5ºC and 25ºC, irrespective of time.

2. Are there different metabolic pathways activated over time in each temperature treatment?

> I can't answer this question by looking at all metabolomes, since my 15ºC control is not specific to any timepoint (control crabs were taken from throughout the experiment to use as a representative sample). Therefore, I would need to split this into two analyses: one for the control (day 0) vs. all timepoints at 25ºC, and one for the control vs. all timepoints at 5ºC.
...
```{r}
scaled.pca <- prcomp(transMetabData[c(6:691)], scale = TRUE, center = TRUE) #Use prcomp to perform a centered and scaled PCA calculation
...
ordiMonteRandTest <- ordi.monte(transMetabData[c(6:691)], ord = "pca", dim = 6, plot = FALSE) #Conduct monte carlo randomization test using 1000 random permutations of the input data.
ordiMonteRandTest
...
Overall, it seems like there is more diversity in the metabolome at 25ºC than at 5ºC or 15ºC. Additionally, PC2 seems to discriminate metabolomes by time. 
...
```{r}
transMetabData[c(6:190)] #All known metabolites
ncol(transMetabData[c(6:190)]) #There are 185 known metabolites
```


```{r}
scaled.pca.id <- prcomp(transMetabData[c(6:190)], scale = TRUE, center = TRUE) #Use prcomp to perform a centered and scaled PCA calculation
...
```{r}
dissim.temp.time <- vegdist(scale(transMetabData[c(6:190)]), "euclidean") #Create euclidean dissimilarity matrix to use for perMANOVA and PERMDISP
```

```{r}
permanova.temp.time <- adonis2(dissim.temp.time ~ as.factor(treatment)*as.numeric(day), data = transMetabData, method = 'eu', by = "terms") #Conduct global PERMANOVA to assess significance of trends in PCA. Scale metabolomics data and assess influence of treatment, day, and treatment x day on metabolite profiles. Include "by = "terms"" to ensure test assess significance for each term.
permanova.temp.time #Significant influence of all factors
```

```{r}
broom::tidy(permanova.temp.time) %>%
...
```{r}
disp.TT.temp <- betadisper(dissim.temp.time, group = as.factor(transMetabData$treatment), type = 'centroid') #Run a beta dispersion model to assess if significant differences are due to differences in group centroid or variance
...
```{r}
disp.TT.day <- betadisper(dissim.temp.time, group = as.factor(transMetabData$day), type = 'centroid') #Run a beta dispersion model to assess if significant differences are due to differences in group centroid or variance
plot(disp.TT.day) #Similar dispersion, but some clear outliers for day 22
boxplot(disp.TT.day) #Similar dispersion differences, outliers seen more easily
anova(disp.TT.day) #Variance is the same between groups. Day significance is due to centroid differences!
```

```{r}
disp.TT.tempDay <- betadisper(dissim.temp.time, group = as.factor(transMetabData$treatment_day), type = 'centroid') #Run a beta dispersion model to assess if significant differences are due to differences in group centroid or variance
plot(disp.TT.tempDay) #Dispersion differences exist, especially with 30_22
boxplot(disp.TT.tempDay) #Very clear dispersion differences for each unique treatment!
