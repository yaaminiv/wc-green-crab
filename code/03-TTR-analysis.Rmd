...
crabMetadata <- read.csv("../../data/crab-metadata.csv", header = TRUE) %>%
  dplyr::select(., c("ID", "treatment", "carapace.width", "carapace.length", "respirometry")) %>%
...
genotypeData <- read.csv("../../data/genotype/2023-crab-genotype.csv", header = TRUE, na.strings = "") %>%
  dplyr::select(., crab.ID, treatment, final.genotype) #Import genotype data. Remove rows without final genotypes
...
```{r}
#Check that crabs aren't missing any variables

is.na(modTTR$sex) %>% sum(.)
is.na(modTTR$integument.cont) %>% sum(.)
is.na(modTTR$carapace.width) %>% sum(.)
is.na(modTTR$weight) %>% sum(.)
is.na(modTTR$missing.swimmer) %>% sum(.)
is.na(modTTR$final.genotype) %>% sum(.)
```
...
I will use a mixed effects model in three steps to assess the impact of various factors on righting response:

1. Assess impact of demographic variables on righting response
2. Include significant demographic variables in model to assess the impacts treatment, day, and their interaction
3. Include significant demographic and explanatory variables in a model with genotype information
...
TTRmodelDV <- lmer(log(TTRavg) ~ as.factor(sex) + integument.cont + carapace.width + weight + as.factor(missing.swimmer) + 
                     (1|crab.ID), 
                   data = modTTR, REML = FALSE) #Run preliminary model with only demographic variables
summary(TTRmodelDV)
```

```{r}
TTRDVnullSex <- lmer(log(TTRavg) ~ integument.cont + carapace.width + weight + as.factor(missing.swimmer) + 
                       (1|crab.ID), 
                     data = modTTR, REML = FALSE) #Run null model without treatment to identify significance of predictor
anova(TTRDVnullSex, TTRmodelDV) #Sex is not significant
```

```{r}
TTRDVnullIntegument <- lmer(log(TTRavg) ~ as.factor(sex) + carapace.width + weight + as.factor(missing.swimmer) + 
                              (1|crab.ID), 
                            data = modTTR, REML = FALSE) #Run null model without treatment to identify significance of predictor
anova(TTRDVnullIntegument, TTRmodelDV) #Integument color is significant
```

```{r}
TTRDVnullCW <- lmer(log(TTRavg) ~ as.factor(sex) + integument.cont + weight + as.factor(missing.swimmer) + 
                      (1|crab.ID), 
                    data = modTTR, REML = FALSE) #Run null model without treatment to identify significance of predictor
anova(TTRDVnullCW, TTRmodelDV) #CW is not significant
```

```{r}
TTRDVnullWeight <- lmer(log(TTRavg) ~ as.factor(sex) + integument.cont + carapace.width + as.factor(missing.swimmer) + 
                          (1|crab.ID), 
                        data = modTTR, REML = FALSE) #Run null model without treatment to identify significance of predictor
anova(TTRDVnullWeight, TTRmodelDV) #Weight is not significant
```

```{r}
TTRDVnullSwimmer <- lmer(log(TTRavg) ~ as.factor(sex) + integument.cont + carapace.width + weight + 
                           (1|crab.ID), 
                         data = modTTR, REML = FALSE) #Run null model without treatment to identify significance of predictor
anova(TTRDVnullSwimmer, TTRmodelDV) #Missing swimmer is significant
```

```{r}
TTRModelComparisons <- rbind(broom::tidy(anova(TTRDVnullSex, TTRmodelDV)),
                             broom::tidy(anova(TTRDVnullIntegument, TTRmodelDV)),
                             broom::tidy(anova(TTRDVnullCW, TTRmodelDV)),
                             broom::tidy(anova(TTRDVnullWeight, TTRmodelDV)),
                             broom::tidy(anova(TTRDVnullSwimmer, TTRmodelDV))) %>%
  mutate(p.adj = p.adjust(p.value, method = "bonferroni")) #Create table for model comparison output
TTRModelComparisons
```

After multiple comparison corrections, only integument color is significant. I will include that in the model.
...
Now that I've run the full model, I want to test the importance of treatment, day, and their interaction as predictors. I'll include all other factors in the null model to account for any variation they may contribute. I'll also re-test the significance of integument color in this context.

```{r}
TTRnullTreatment <- lmer(log(TTRavg) ~ day + integument.cont + 
                           (1|crab.ID), 
                         data = modTTR, REML = FALSE) #Run null model without treatment to identify significance of predictor
...
```{r}
TTRnullDay <- lmer(log(TTRavg) ~ treatment + integument.cont + 
                     (1|crab.ID), data = modTTR, REML = FALSE) #Run null model without day to identify significance of predictor
anova(TTRnullDay, TTRmodel) #Day is a significant predictor
```

```{r}
TTRnullInteraction <- lmer(log(TTRavg) ~ treatment + day + integument.cont +
                             (1|crab.ID), data = modTTR, REML = FALSE) #Run model without interaction to identify significance of predictor. The full model constructed does not include an interaction
anova(TTRnullInteraction, TTRmodel) #Interaction is a significant predictor
```

```{r}
TTRnullIntegument2 <- lmer(log(TTRavg) ~ treatment + day + treatment:day +
                             (1|crab.ID), data = modTTR, REML = FALSE) #Run model without interaction to identify significance of predictor. The full model constructed does not include an interaction
anova(TTRnullIntegument2, TTRmodel) #Integument color is not a significant predictor
```
...
TTRModelComparisons <- rbind(broom::tidy(anova(TTRDVnullSex, TTRmodelDV)),
                             ...
                             broom::tidy(anova(TTRDVnullWeight, TTRmodelDV)),
                             broom::tidy(anova(TTRDVnullSwimmer, TTRmodelDV)),
                             broom::tidy(anova(TTRnullTreatment, TTRmodel)),
                             broom::tidy(anova(TTRnullDay, TTRmodel)),
                             broom::tidy(anova(TTRnullInteraction, TTRmodel)),
                             broom::tidy(anova(TTRnullIntegument2, TTRmodel))) %>%
  mutate(p.adj = p.adjust(p.value, method = "bonferroni")) #Create table for model comparison output
TTRModelComparisons
```
...
Since integument color is not significant when included just with temperature, day, and their interaction, I will compare AIC between a model that has integument color and a model that does not.

```{r}
AIC(TTRnullIntegument2, TTRmodel) #The model that does not include integument color has a slightly lower AIC than a model that includes integument color. I will remove integument color from the final model.
```

```{r}
TTRmodelFinal <- TTRnullIntegument2 #The final model includes treatment, day, and their interaction as fixed effects and crab ID as a random effect.
summary(TTRmodelFinal)
```

### Genotype analysis

I will now investigate the impact of genotype-associated variables on righting response. The genotype variables are as follows:

1. Genotype
2. Presence of "C" (cold-adapted allele)
3. Presence of "T" (warm-adapted allele)
...
TTRmodelGenotype <- lmer(log(TTRavg) ~ treatment + day + treatment:day + 
                           final.genotype +
                           (1|crab.ID), 
                         data = modTTR, REML = FALSE) #Run model with treatment, timepoint, their interaction, sex, integument color, carapace width, weight, and missing swimmer information as fixed effects, and individual as a random effect. Change the likelihood estimator (REML = FALSE) to compare models with LRT later.
anova(TTRmodelFinal, TTRmodelGenotype) #Full genotype is not a significant predictor
```

```{r}
TTRmodelpresenceC <- lmer(log(TTRavg) ~ treatment + day + treatment:day + 
                            presenceC +
                            (1|crab.ID), 
                          data = modTTR, REML = FALSE) #Run model with treatment, timepoint, their interaction, sex, integument color, carapace width, weight, and missing swimmer information as fixed effects, and individual as a random effect. Change the likelihood estimator (REML = FALSE) to compare models with LRT later.
anova(TTRmodelFinal, TTRmodelpresenceC) #Presence of C allele is not a significant predictor
```

```{r}
TTRmodelpresenceT <- lmer(log(TTRavg) ~ treatment + day + treatment:day + 
                            presenceT +
                            (1|crab.ID), 
                          data = modTTR, REML = FALSE) #Run model with treatment, timepoint, their interaction, sex, integument color, carapace width, weight, and missing swimmer information as fixed effects, and individual as a random effect. Change the likelihood estimator (REML = FALSE) to compare models with LRT later.
anova(TTRmodelFinal, TTRmodelpresenceT) #Presence of T allele is not a significant predictor
```
...
TTRModelComparisons <- rbind(broom::tidy(anova(TTRDVnullSex, TTRmodelDV)),
                             ...
                             broom::tidy(anova(TTRnullInteraction, TTRmodel)),
                             broom::tidy(anova(TTRnullIntegument2, TTRmodel)),
                             broom::tidy(anova(TTRmodelFinal, TTRmodelGenotype)),
                             broom::tidy(anova(TTRmodelFinal, TTRmodelpresenceC)),
                             broom::tidy(anova(TTRmodelFinal, TTRmodelpresenceT))) %>%
  mutate(p.adj = p.adjust(p.value, method = "bonferroni")) #Create table for model comparison output
TTRModelComparisons
```

```{r}
write.csv(TTRModelComparisons, "ttr-model-comparison-stat-output.csv", quote = FALSE, row.names = FALSE) #Save dataframe as a csv
```
...
```{r}
var.lmer <- c(0.08235, 0.16730) #Save variances of random effects as a new vector (crab ID, residual)
percentvar.lmer <- (100*var.lmer)/sum(var.lmer) #Calculate percent variances
percentvar.lmer[1] #Crab ID accounts for 32.98618% of variance
```

### Check assumptions

```{r}
hist(residuals(TTRmodelFinal)) #Residuals normally distributed

plot(fitted(TTRmodelFinal), residuals(TTRmodelFinal)) #Borderline heteroskedastic but probably okay? Lack of dispersion across x-axis is likely related to discrete variables
...
### Posthoc test for significant predictors
...
```{r}
TTRmodelFinal_treatment <- emmeans(TTRmodelFinal, pairwise ~ treatment | day, at = list(timepoint = c(0, 1, 2, 7, 12, 14, 19, 21, 26, 28, 42)), adjust = "tukey", data = modTTR) #Use emmeans to obtain pairwise treatment differences at the timepoints listed. 
TTRmodelFinal_treatment$contrasts %>% broom::tidy(.) #Extract the contrast information to look at pairwise test output. Clean output using broom.
TTRmodelFinal_treatment$contrasts %>% 
  broom::tidy(.) %>%
  write.csv("tukey-test-avgTTR-treatment.csv", quote = FALSE, row.names = FALSE) #Save output as a csv.
```
...
```{r}
TTRmodelFinal_treatment_slope <- emtrends(TTRmodelFinal, pairwise ~ treatment, var = "day", data = modTTR) #Obtain simple slopes for the interaction between each temperature and time. Test if the effect of time significantly differs between the two treatments
TTRmodelFinal_treatment_slope$emtrends #Different slopes for each temperature (makes sense because there was an interaction)
TTRmodelFinal_treatment_slope$contrasts #Significance test for the different slopes. Slopes were significantly different between the control and the cold and warm conditions
```

#### Day

I want to understand how TTR varied by day within different treatments. There is no way to do this with my original model since day was (rightfully) coded as a continuous variable. In order to get pairwise estimates, I will need to refactor day as a categorical variable, rerun the model, and then use `emmeans` on this revised model.

```{r}
modTTR$day_factor <- as.factor(modTTR$day) #Code day as a factor
TTRmodelFinal_factor <- lmer(log(TTRavg) ~ treatment + day_factor + treatment:day_factor + 
                               (1|crab.ID), 
                             data = modTTR, REML = FALSE) #Run the original model, but use day as a factor
TTRmodelFinal_day <- emmeans(TTRmodelFinal_factor, pairwise ~ day_factor | treatment, adjust = "tukey", data = modTTR) #Rerun emmeans using day as a factor
TTRmodelFinal_day$contrasts %>% broom::tidy(.) #Extract contrasts to look at pairwise output. Clean output using broom.
TTRmodelFinal_day$contrasts %>%
  broom::tidy(.) %>%
  write.csv("tukey-test-avgTTR-day.csv", quote = FALSE, row.names = FALSE) #Save output as a csv.
```
...
mainPlot <- modTTR %>%
  ...
  scale_shape_manual(values = c(15, 19, 17),
                     ...
                     breaks = c("05C", "15C", "25C"),
                     labels = c("5", "15", "25")) +
  geom_text(x = 1.5, y = 22,
            label = "bolditalic(Temperature) * \": \" * chi^2 * \" = 239.28, p < 0.0001\"",
            hjust = 0,
            parse = TRUE,
            inherit.aes = FALSE,
            color = "black",
            size = 3.5) +
  geom_text(x = 1.5, y = 21,
            label = "bolditalic(Day) * \": \" * chi^2 * \" = 148.24, p < 0.0001\"",
            hjust = 0,
            parse = TRUE,
            inherit.aes = FALSE,
            color = "black",
            size = 3.5) +
  geom_text(x = 1.5, y = 20,
            label = "bolditalic(Interaction) * \": \" * chi^2 * \" = 147.41, p < 0.0001\"",
            hjust = 0,
            parse = TRUE,
            inherit.aes = FALSE,
            color = "black",
            size = 3.5) +
  theme_classic(base_size = 15) #Select day, treatment, and TTRavg data. Replace numbers with characters to facilitate better ordering. Plot average TTR for each crab in a boxplot. Do not show outliers with the OG boxplot, but add them in with geom_jitter. Add horizontal line for boxplot outlier threshold. Assign colors to each treatment. Annotate with stat output. Increase base font size.
...
insetPlot <-  modTTR %>%
  ...
  scale_x_discrete(name = "",
                   breaks = c("0", "01", "02", "07", "12", "14", "19", "21", "26", "28", "42"),
                   labels = c("0", "1", "2", "7", "12", "14", "19", "21", "26", "28", "42")) +
...
plotWithInset <- ggdraw() +
  draw_plot(mainPlot) +
  draw_plot(insetPlot, x = 0.09, y = 0.67, width = 0.65, height = 0.23)
...
nrow(modTTRSubsetDiffStats) #82 data points
```
...
I'm not going to re-test demographic variables since those were not significant in the full dataset.
...
```{r}
TTRnullDiffInteraction <- lmer(TTRdiff ~ treatment + day + 
                                 (1|crab.ID), 
                               data = modTTRSubsetDiffStats, REML = FALSE) #Run model without interaction to identify significance of predictor. The full model constructed does not include an interaction
...
TTRDiffModelComparisons <- rbind(broom::tidy(anova(TTRnullDiffTreatment, TTRDiffmodel)),
                                 ...
                                 broom::tidy(anova(TTRnullDiffInteraction, TTRDiffmodel))) %>%
  mutate(p.adj = p.adjust(p.value, method = "bonferroni")) #Create table for model comparison output
TTRDiffModelComparisons
```

```{r}
write.csv(TTRDiffModelComparisons, "ttr-diff-comparison-stat-output.csv", quote = FALSE, row.names = FALSE) #Save model output
```
...
anova(TTRnullDiffDay, TTRdiffModelGenotype) #Final genotype is not a signficiant predictor of difference in TTR
```
```{r}
TTRdiffModelpresenceC <- lmer(TTRdiff ~ treatment + 
                                presenceC +
                                (1|crab.ID), 
                              data = modTTRSubsetDiffStats, REML = FALSE) #Run model with treatment, timepoint, their interaction, sex, integument color, carapace width, weight, and missing swimmer information as fixed effects, and individual as a random effect. Change the likelihood estimator (REML = FALSE) to compare models with LRT later.
anova(TTRnullDiffDay, TTRdiffModelpresenceC) #C presence is not a signficiant predictor of difference in TTR
```

```{r}
TTRdiffModelpresenceT <- lmer(TTRdiff ~ treatment + 
                                presenceT +
                                (1|crab.ID), 
                              data = modTTRSubsetDiffStats, REML = FALSE) #Run model with treatment, timepoint, their interaction, sex, integument color, carapace width, weight, and missing swimmer information as fixed effects, and individual as a random effect. Change the likelihood estimator (REML = FALSE) to compare models with LRT later.
anova(TTRnullDiffDay, TTRdiffModelpresenceT) #C presence is not a signficiant predictor of difference in TTR
```
...
TTRDiffModelComparisons <- rbind(broom::tidy(anova(TTRnullDiffTreatment, TTRDiffmodel)),
                                 broom::tidy(anova(TTRnullDiffDay, TTRDiffmodel)),
                                 broom::tidy(anova(TTRnullDiffInteraction, TTRDiffmodel)),
                                 broom::tidy(anova(TTRnullDiffDay, TTRdiffModelGenotype)),
                                 broom::tidy(anova(TTRnullDiffDay, TTRdiffModelpresenceC)),
                                 broom::tidy(anova(TTRnullDiffDay, TTRdiffModelpresenceT))) %>%
  mutate(p.adj = p.adjust(p.value, method = "bonferroni")) #Create table for model comparison output
TTRDiffModelComparisons
```
...
head(modTTRQ10)
length(modTTRQ10$crab.ID %>% unique(.)) #29 crabs
```
