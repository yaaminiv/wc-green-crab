...
all_MO2_slope_results_demTTRdata_genotype <- all_MO2_slope_results_demTTRdata %>%
  left_join(., genotypeData, by = c("crab.ID", "treatment")) #Join dataframes by crab ID and treatment. 
head(all_MO2_slope_results_demTTRdata_genotype) #Confirm left_join
```
...
lmAllSlopeResultsSig <- aov(log(-slope_nmol_hr_corrected) ~ treatment + as.numeric(day) + 
    missing.legs + final.genotype + treatment:as.numeric(day), data = all_MO2_slope_results_demTTRdata_genotype) #Run the most significant model identified by step
...
plot(fitted(lmAllSlopeResultsSig), residuals(lmAllSlopeResultsSig)) #Slight heteroskedasticity
