...
```{r}
nzv_indices <- nearZeroVar((transMetabData %>% 
                              filter(., day != "0"))[c(14:198)] %>%
                             log(.), 
                           saveMetrics = FALSE) #Check to see if any variables have near-zero variance.
nzv_indices #There aren't any columns with near-zero variance!
```
...
X_tempTime <- transMetabData %>% 
  ...
  log(.) #Assign known metabolite data to the X dataframe after log transformation
Y_tempTime <- as.factor((transMetabData %>% filter(., day != "0"))$treatment_day) #Assign treatment data to the Y dataframe
```

```{r}
treatmentday.known.plsda <- plsda(x = X_tempTime, y = Y_tempTime, ncomp = 10) #Initially set a higher number for the number of components since we will be testing this
