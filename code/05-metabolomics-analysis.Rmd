...
ordiMonteRandTest <- ordi.monte(transMetabData[c(7:692)], ord = "pca", dim = 6, plot = FALSE) #Conduct monte carlo randomization test using 1000 random permutations of the input data.
ordiMonteRandTest
...
```{r}
transMetabData[c(7:191)] #All known metabolites
ncol(transMetabData[c(7:191)]) #There are 185 known metabolites
```

```{r}
scaled.pca.T.id <- prcomp(transMetabData[c(7:191)], scale = TRUE, center = TRUE) #Use prcomp to perform a centered and scaled PCA calculation
...
```{r}
dissim.T <- vegdist(scale(transMetabData[c(7:692)]), "euclidean") #Create euclidean dissimilarity matrix to use for perMANOVA and PERMDISP
```
...
```{r}
dissim.T.id <- vegdist(scale(transMetabData[c(7:191)]), "euclidean") #Create euclidean dissimilarity matrix to use for perMANOVA and PERMDISP
```
...
```{r}
(transMetabData %>% filter(., day != "0"))[c(7:692)] #Data set without control crabs (day 0)
```


```{r}
scaled.pca.TT <- prcomp((transMetabData %>% filter(., day != "0"))[c(7:692)], scale = TRUE, center = TRUE) #Use prcomp to perform a centered and scaled PCA calculation
...
```{r}
(transMetabData %>% filter(., day != "0"))[c(7:191)] #Data set without control crabs (day 0)
```


```{r}
scaled.pca.TT.id <- prcomp((transMetabData %>% filter(., day != "0"))[c(7:191)], scale = TRUE, center = TRUE) #Use prcomp to perform a centered and scaled PCA calculation
...
```{r}
(transMetabData %>% filter(., day != "0"))[c(7:692)]
```

```{r}
dissim.TT <- vegdist(scale((transMetabData %>% filter(., day != "0"))[c(7:692)]), "euclidean") #Create euclidean dissimilarity matrix to use for perMANOVA and PERMDISP
```
...
```{r}
(transMetabData %>% filter(., day != "0"))[c(7:191)]
```

```{r}
dissim.TT.id <- vegdist(scale((transMetabData %>% filter(., day != "0"))[c(7:191)]), "euclidean") #Create euclidean dissimilarity matrix to use for perMANOVA and PERMDISP
```
