...
```
In this model, day is not significant but it's included in the model. I'm going to see what happens when I remove it.

```{r}
lmAllSlopeResultsSigMinusDay <- glm(log(-slope_nmol_hr_corrected) ~ treatment + final.genotype + 
                                      treatment:final.genotype,
                                    data = (all_MO2_slope_results_demTTRdata_genotype %>% filter(., blank_ratio < 0.10))) #Same model, minus day
AIC(lmAllSlopeResultsSig, lmAllSlopeResultsSigMinusDay) #AIC is lower for the model with day
```
...
```{r}
lmBinarySlopeResultsSigMinusTreat <- glm(log(-slope_nmol_hr_corrected) ~ as.numeric(day) + hasC + hasT +
                                 as.numeric(day):hasC, 
                               data = (all_MO2_slope_results_demTTRdata_genotype %>% filter(., blank_ratio < 0.10))) #Same model, minus treatment
lmBinarySlopeResultsSigMinushasT <- glm(log(-slope_nmol_hr_corrected) ~ treatment + as.numeric(day) + hasC +
...
#### Plot by binary genotype

Since presence of the T allele was marginally significant in the ANOVA, I want to plot by T presence instead of full genotype to see if that makes any patterns clearer. My post-hoc tests show that presence of the T allele increased oxygen consumption.
