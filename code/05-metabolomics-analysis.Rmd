...
t.test_CC_5Cv25C %>%
  ...
  left_join(., (rawMetabolomicsData %>% 
                  dplyr::rename(metabolite = BinBase.name)), 
            by = "metabolite") %>%
  mutate(., higherAbundanceIn = case_when(estimate1 > estimate2 ~ "25C_CC",
                                          estimate1 < estimate2 ~ "5C_CC")) %>%
  dplyr::select(., metabolite, PubChem, KEGG, higherAbundanceIn) %>%
  unique(.) %>%
...
t.test_CT_5Cv25C %>%
  ...
  left_join(., (rawMetabolomicsData %>% 
                  dplyr::rename(metabolite = BinBase.name)), 
            by = "metabolite") %>%
  mutate(., higherAbundanceIn = case_when(estimate1 > estimate2 ~ "25C_CT",
                                          estimate1 < estimate2 ~ "5C_CT")) %>%
  dplyr::select(., metabolite, PubChem, KEGG, higherAbundanceIn) %>%
  unique(.) %>%
...
t.test_TT_5Cv25C %>%
  ...
  left_join(., (rawMetabolomicsData %>% 
                  dplyr::rename(metabolite = BinBase.name)), 
            by = "metabolite") %>%
  mutate(., higherAbundanceIn = case_when(estimate1 > estimate2 ~ "25C_TT",
                                          estimate1 < estimate2 ~ "5C_TT")) %>%
  dplyr::select(., metabolite, PubChem, KEGG, higherAbundanceIn) %>%
  unique(.) %>%
