...
graph.1 <- crab.dat.main %>% 
  filter(day == "0" | day == "01" | day == "07" | day == "21" | day == "42") %>% 
  ggplot(., aes(x = day, y = carapace.width, color = treatment)) + geom_boxplot() + labs(x = "Timepoint (Day)", y = "Caraoace Wdth (mm)") + theme_classic() + scale_color_manual(values = c(plotColors[2], plotColors[1], plotColors[3]), 
                     name = "Temperature (ºC)",
                     breaks = c("15C", "25C", "5C"),
                     labels = c("15", "25", "5"))
```
```{r}
# trying to make a bar graph instead
bar.graph <- ggplot(crab.dat.main, aes(x=day)) + geom_histogram(binwidth=2,colour="orange")
bar.graph + facet_grid(integument.color ~ sex.x)
```
```{r}
# remade the second bar graph
bar.graph.two <- ggplot(crab.dat.main, aes(x=day, y=) + geom_bar()
bar.graph.two + facet_grid(sex.x ~ treatment, scales= "free", space= "free")
```
```{r}
# making graphs for weights per treatment
crab.dat.main %>% 
  filter(day == "0" | day == "01" | day == "07" | day == "21" | day == "42") %>% 
  ggplot(., aes(x = day, y = weight, color = treatment)) + geom_boxplot() + labs(x = "Timepoint (Day)", y = "Average Weight (g)") + theme_classic()
...
```
```{r}
# installing color brewer to fix the graph
...
```
```{r}
# running ANOVA test to see example
ANOVAresult <- aov(formula = carapace.width ~ weight, data = crab.dat.main)
```
```{r}
