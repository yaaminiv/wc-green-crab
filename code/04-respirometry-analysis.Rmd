...
```{r}
lmAllSlopeDV <- lmer(log(-slope_nmol_hr_corrected) ~ sex + weight + carapace.width + integument.cont + missing.legs +
                       (1|crab.ID), 
                     data = (all_MO2_slope_results_demTTRdata_genotype %>% filter(., blank_ratio < 0.10)), REML = FALSE) #Examine the influence of treatment, day, genotype, and various demographic factors on respiration rate. Remove outliers based on blank ratios prior to analysis.
summary(lmAllSlopeDV) #Significant impact of treatment and treatment:day on respiration
```

```{r}
lmAllSlopeDVSex <- lmer(log(-slope_nmol_hr_corrected) ~ weight + carapace.width + integument.cont + missing.legs +
                          (1|crab.ID), 
                        data = (all_MO2_slope_results_demTTRdata_genotype %>% filter(., blank_ratio < 0.10)), REML = FALSE) #Examine the influence of treatment, day, genotype, and various demographic factors on respiration rate. Remove outliers based on blank ratios prior to analysis.
anova(lmAllSlopeDV, lmAllSlopeDVSex) #Sex is not a significant predictor
```

```{r}
lmAllSlopeDVWeight <- lmer(log(-slope_nmol_hr_corrected) ~ sex + carapace.width + integument.cont + missing.legs +
                             (1|crab.ID), 
                           data = (all_MO2_slope_results_demTTRdata_genotype %>% filter(., blank_ratio < 0.10)), REML = FALSE) #Examine the influence of treatment, day, genotype, and various demographic factors on respiration rate. Remove outliers based on blank ratios prior to analysis.
anova(lmAllSlopeDV, lmAllSlopeDVWeight) #Weight is not a significant predictor
```

```{r}
lmAllSlopeDVCW <- lmer(log(-slope_nmol_hr_corrected) ~ sex + weight + integument.cont + missing.legs +
                         (1|crab.ID), 
                       data = (all_MO2_slope_results_demTTRdata_genotype %>% filter(., blank_ratio < 0.10)), REML = FALSE) #Examine the influence of treatment, day, genotype, and various demographic factors on respiration rate. Remove outliers based on blank ratios prior to analysis.
anova(lmAllSlopeDV, lmAllSlopeDVCW) #CW is not a significant predictor
```

```{r}
lmAllSlopeDVIntegument <- lmer(log(-slope_nmol_hr_corrected) ~ sex + weight + carapace.width + missing.legs +
                                 (1|crab.ID), 
                               data = (all_MO2_slope_results_demTTRdata_genotype %>% filter(., blank_ratio < 0.10)), REML = FALSE) #Examine the influence of treatment, day, genotype, and various demographic factors on respiration rate. Remove outliers based on blank ratios prior to analysis.
anova(lmAllSlopeDV, lmAllSlopeDVIntegument) #Integument color is a marginally significant predictor, will likely not be significant after multiple test correction
```

```{r}
lmAllSlopeDVMissing <- lmer(log(-slope_nmol_hr_corrected) ~ sex + weight + carapace.width + integument.cont +
                              (1|crab.ID), 
                            data = (all_MO2_slope_results_demTTRdata_genotype %>% filter(., blank_ratio < 0.10)), REML = FALSE) #Examine the influence of treatment, day, genotype, and various demographic factors on respiration rate. Remove outliers based on blank ratios prior to analysis.
anova(lmAllSlopeDV, lmAllSlopeDVMissing) #Missing legs are not a significant predictor
```
```{r}
rbind(broom::tidy(anova(lmAllSlopeDV, lmAllSlopeDVSex)),
      broom::tidy(anova(lmAllSlopeDV, lmAllSlopeDVWeight)),
      broom::tidy(anova(lmAllSlopeDV, lmAllSlopeDVCW)),
      broom::tidy(anova(lmAllSlopeDV, lmAllSlopeDVIntegument)),
      broom::tidy(anova(lmAllSlopeDV, lmAllSlopeDVMissing))) %>%
  mutate(p.adj = p.adjust(p.value, method = "bonferroni")) #Combine all statistical test information and adjust for multiple comparisons
```

Based on this, there are no demographic variables significantly impacting oxygen consumption.

#### Assess impact of experimental variables (treatment, day, and their interaction)

```{r}
lmAllSlopeResults <- lmer(log(-slope_nmol_hr_corrected) ~ treatment + as.numeric(day) + treatment:as.numeric(day) +
                            (1|crab.ID), 
                          data = (all_MO2_slope_results_demTTRdata_genotype %>% filter(., blank_ratio < 0.10)), REML = FALSE) #Examine the influence of treatment, day, genotype, and various demographic factors on respiration rate. Remove outliers based on blank ratios prior to analysis.
...
```{r}
lmAllSlopeResultsNullTemperature <- lmer(log(-slope_nmol_hr_corrected) ~ as.numeric(day) +
                                           (1|crab.ID), 
                                         data = (all_MO2_slope_results_demTTRdata_genotype %>% filter(., blank_ratio < 0.10)), REML = FALSE) #Create a null model without the factor of interest
anova(lmAllSlopeResultsNullTemperature, lmAllSlopeResults) #Compare models. Temperature is significant
```

```{r}
lmAllSlopeResultsNullDay <- lmer(log(-slope_nmol_hr_corrected) ~ treatment + 
                                   (1|crab.ID), 
                                 data = (all_MO2_slope_results_demTTRdata_genotype %>% filter(., blank_ratio < 0.10)), REML = FALSE) #Create a null model without the factor of interest
anova(lmAllSlopeResultsNullDay, lmAllSlopeResults) #Compare models. Day is significant, but may not be after multiple test correction
```

```{r}
lmAllSlopeResultsNullInteraction <- lmer(log(-slope_nmol_hr_corrected) ~ treatment + as.numeric(day) +
                                           (1|crab.ID), 
                                         data = (all_MO2_slope_results_demTTRdata_genotype %>% filter(., blank_ratio < 0.10)), REML = FALSE) #Create a null model without the factor of interest
anova(lmAllSlopeResultsNullInteraction, lmAllSlopeResults) #Compare models. Interaction is significant, but may not be after multiple test correction
```

```{r}
baseModelComparisons <- rbind(broom::tidy(anova(lmAllSlopeResultsNullTemperature, lmAllSlopeResults)),
                              broom::tidy(anova(lmAllSlopeResultsNullDay, lmAllSlopeResults)),
                              broom::tidy(anova(lmAllSlopeResultsNullInteraction, lmAllSlopeResults))) #Create table for model comparison output
baseModelComparisons
```
...
rbind(broom::tidy(anova(lmAllSlopeDV, lmAllSlopeDVSex)),
      ...
      broom::tidy(anova(lmAllSlopeDV, lmAllSlopeDVIntegument)),
      broom::tidy(anova(lmAllSlopeDV, lmAllSlopeDVMissing)),
      broom::tidy(anova(lmAllSlopeResultsNullTemperature, lmAllSlopeResults)),
      broom::tidy(anova(lmAllSlopeResultsNullDay, lmAllSlopeResults)),
      broom::tidy(anova(lmAllSlopeResultsNullInteraction, lmAllSlopeResults))) %>%
  mutate(p.adj = p.adjust(p.value, method = "bonferroni"))
```

Day and treatment are not significant after multiple test correction.

#### Assess impact of genotype

Now that I know what parameters should be in the base model, I will add genotype to see if that makes a difference.

```{r}
lmAllSlopeResultsNullDay #Model that just has temperature which is the best model for comparing genotype variable addition
```


```{r}
lmAllSlopeResultsGenotype <- lmer(log(-slope_nmol_hr_corrected) ~ treatment +
                                    final.genotype +
                                    (1|crab.ID), 
                                  data = (all_MO2_slope_results_demTTRdata_genotype %>% filter(., blank_ratio < 0.10)), REML = FALSE) #Examine the influence of treatment, day, genotype, and various demographic factors on respiration rate. Remove outliers based on blank ratios prior to analysis.
anova(lmAllSlopeResultsGenotype, lmAllSlopeResultsNullDay) #No impact of genotype on oxygen consumption
```
...
```{r}
lmAllSlopeResultsHasC <- lmer(log(-slope_nmol_hr_corrected) ~ treatment +
                                hasC +
                                (1|crab.ID), 
                              data = (all_MO2_slope_results_demTTRdata_genotype %>% filter(., blank_ratio < 0.10)), REML = FALSE) #Create a null model without the factor of interest
anova(lmAllSlopeResultsHasC, lmAllSlopeResultsNullDay) #Compare models. C allele is not significant
```

```{r}
lmAllSlopeResultsHasT <- lmer(log(-slope_nmol_hr_corrected) ~ treatment +
                                hasT +
                                (1|crab.ID), 
                              data = (all_MO2_slope_results_demTTRdata_genotype %>% filter(., blank_ratio < 0.10)), REML = FALSE) #Create a null model without the factor of interest
anova(lmAllSlopeResultsHasT, lmAllSlopeResultsNullDay) #Compare models. T allele is not significant
```

```{r}
modelStatOutput <- rbind(broom::tidy(anova(lmAllSlopeDV, lmAllSlopeDVSex)),
      broom::tidy(anova(lmAllSlopeDV, lmAllSlopeDVWeight)),
      broom::tidy(anova(lmAllSlopeDV, lmAllSlopeDVCW)),
      broom::tidy(anova(lmAllSlopeDV, lmAllSlopeDVIntegument)),
      broom::tidy(anova(lmAllSlopeDV, lmAllSlopeDVMissing)),
      ...
      broom::tidy(anova(lmAllSlopeResultsNullDay, lmAllSlopeResults)),
      broom::tidy(anova(lmAllSlopeResultsNullInteraction, lmAllSlopeResults)),
      broom::tidy(anova(lmAllSlopeResultsNullDay, lmAllSlopeResultsGenotype)),
      broom::tidy(anova(lmAllSlopeResultsNullDay, lmAllSlopeResultsHasC)),
      broom::tidy(anova(lmAllSlopeResultsNullDay, lmAllSlopeResultsHasT))) %>%
  mutate(p.adj = p.adjust(p.value, method = "bonferroni"))
modelStatOutput
```

```{r}
write.csv(modelStatOutput, "model-comparison-stat-output.csv", quote = FALSE, row.names = FALSE) #Save model output
```

#### Final model

```{r}
lmAllSlopeResultsFinal <- lmAllSlopeResultsNullDay #Save final model
summary(lmAllSlopeResultsFinal)
```

```{r}
var.lmer <- c(0.006216, 0.07884) #Save variances of random effects as a new vector (crab ID, residual)
percentvar.lmer <- (100*var.lmer)/sum(var.lmer) #Calculate percent variances
percentvar.lmer[1] #Crab ID accounts for 7.308126% of variance
```

#### Check assumptions

```{r}
hist(residuals(lmAllSlopeResultsFinal)) #Residuals normally distributed

plot(fitted(lmAllSlopeResultsFinal), residuals(lmAllSlopeResultsFinal)) #Very heteroskedastic...
abline(h = 0, lty = 2, col = "grey")
```

#### Posthoc test for significant predictors

There was significant impact of treatment on oxygen consumption. I will investigate these effects using post-hoc tests with `emmeans`.

```{r}
lmAllSlopeResultsFinal_treatment <- emmeans(lmAllSlopeResultsFinal, pairwise ~ treatment, adjust = "tukey", data = (all_MO2_slope_results_demTTRdata_genotype %>% filter(., blank_ratio < 0.10))) #Use emmeans to obtain pairwise treatment differences at the timepoints listed. 
lmAllSlopeResultsFinal_treatment$contrasts %>% broom::tidy(.) #Extract the contrast information to look at pairwise test output. Clean output using broom.
lmAllSlopeResultsFinal_treatment$contrasts %>% 
  broom::tidy(.) %>%
  write.csv("tukey-test-oxygenConsumptionRate-treatment.csv", quote = FALSE, row.names = FALSE) #Save output as a csv.
```
...
hasT_labels <- c("N" = "CC",
                 "Y" = "CT or TT") #Assign labels for facets
```

- add test results (temp, day, and interaction)
...
all_MO2_slope_results_reorder %>%
  ...
  scale_shape_manual(values = c(15, 19, 17),
                     ...
                     breaks = c("05C", "15C", "25C"),
                     labels = c("5", "15", "25")) +
  geom_text(x = 4.5, y = 25000,
            label = "bolditalic(Temperature) * \": \" * chi^2 * \" = 61.03, p < 0.0001\"",
            hjust = 0,
            parse = TRUE,
            inherit.aes = FALSE,
            color = "black",
            size = 3.5) +
  geom_text(x = 4.5, y = 24200,
            label = "bolditalic(Day) * \": \" * chi^2 * \" = 9.26, p = 0.29\"",
            hjust = 0,
            parse = TRUE,
            inherit.aes = FALSE,
            color = "black",
            size = 3.5) +
  geom_text(x = 4.5, y = 24200 - 800,
            label = "bolditalic(Interaction) * \": \" * chi^2 * \" = 6.15, p < 0.51\"",
            hjust = 0,
            parse = TRUE,
            inherit.aes = FALSE,
            color = "black",
...
all_MO2_slope_results_reorder %>%
  ...
  geom_boxplot() + geom_point(aes(shape = treatment), size = 2) +
  xlab("") + ylab("Oxygen Consumption (nmol/hr)") +
  facet_grid(hasT ~ day, labeller = labeller (day = facet_labels,
                                              hasT = hasT_labels)) +
  scale_x_discrete(breaks = c("05C", "15C", "25C"),
                   labels = c("", "", "")) +
  scale_y_continuous(breaks = seq(0, 25000, 5000)) +
  ...
  theme_classic(base_size = 15) + theme(axis.ticks.x = element_blank(),
                                        ...
                                        strip.text.x = element_text(size = 15),
                                        strip.background = element_rect(colour = "white"))
ggsave("figures/multipanel-oxygen-consumption-corrected-hasT.pdf", height = 8.5, width = 11)
#Create a boxplot for oxygen consumption (µmol/hr). Use -slope_nmol_hr so the larger the slope, the faster the oxygen consumption over the hour. Facet wrap and use the labeller to relabel facets. Add consistent color and shape scales.
```
