...
lmAllSlopeResultsSig <- glm(log(-slope_nmol_hr_corrected) ~ treatment + as.numeric(day) + 
    sex + integument.cont + treatment:as.numeric(day), data = (all_MO2_slope_results_demTTRdata_genotype %>% 
    filter(., blank_ratio < 0.1))) #Run the most significant model identified by step
summary(lmAllSlopeResultsSig) #Significant impact of treatment*day. Genotype not included
```
...
```{r}
lmBinarySlopeResults <- glm(log(-slope_nmol_hr_corrected) ~ treatment*as.numeric(day) + hasC + hasT + 
                              sex + weight + carapace.width + integument.cont + missing.legs, 
...
lmBinarySlopeResultsSig <- glm(log(-slope_nmol_hr_corrected) ~ treatment + as.numeric(day) + 
    hasT + sex + integument.cont + treatment:as.numeric(day), 
    data = (all_MO2_slope_results_demTTRdata_genotype %>% filter(., 
        blank_ratio < 0.1))) #Run the most significant model identified by step
summary(lmBinarySlopeResultsSig) #Significant impact of treatment*day. Genotype (T presence) included but not significant
```

In this model, treatment and day are significant, and only the interaction between treatment and day for 5ºC is significant. Also, presence of the T allele is significant, but presence of C is not included in the model.

```{r}
lmBinarySlopeResultsSigMinushasT <- glm(log(-slope_nmol_hr_corrected) ~treatment + as.numeric(day) + sex + integument.cont + treatment:as.numeric(day), 
    data = (all_MO2_slope_results_demTTRdata_genotype %>% filter(., 
        blank_ratio < 0.1))) #Same model, minus hasT
AIC(lmBinarySlopeResultsSig, lmBinarySlopeResultsSigMinushasT) #Step model is still the most significant
```
