...
```{r}
crab_metadata %>% #Uses this data frame for the graph 
  ggplot(., aes(x = as.character(treatment), #tells us what column we are pulling data from to make the x-axis 
                y = `carapace-width`, #tells us what column we are taking data from to use for Y 
                color = treatment, #changes the color for each treatment 
                shape = treatment)) + #Puts different shapes for the two treatments
  ...
  labs(x = "Temperature (ºC)", y = "Carapace Width (mm)") + #Allows us to name the X and Y axes 
  scale_color_manual(values = c("green", "purple", "lightblue")) + #Allows us to pick the colors for out two crab populations 
