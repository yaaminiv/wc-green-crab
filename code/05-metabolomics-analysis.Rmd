...
pcaPlot4 <- autoplot(scaled.pca.TT, data = (transMetabData %>% filter(., day != "0")), x = 1, y = 2,
  ...
  scale_color_manual(values = c("lightblue4", "lightblue2", "steelblue1", plotColors[3], 
                     ...
                     labels = c("5ºC at Day 1", "5ºC at Day 7", "5ºC at Day 26", "5ºC at Day 42",
                                "25ºC at Day 1", "25ºC at Day 7", "25ºC at Day 26", "25ºC at Day 42"),
                     name = "Treatment") +
...
anova(disp.TT.tempDay) #Variance is different between groups. Interaction significance is due to centroid and dispersion differences
```
...
```{r}
pdf("PLSDA/figures/known-metabolites-cimPlot-treatmentDay.pdf", height = 8.5, width = 11)
cim(treatmentDay.known.plsda.tuned, mapping = "Y", row.names = (transMetabData %>% filter(., day != 0))$treatment_day, xlab = "Metabolites", ylab = "Temperature (ºC) x Day", margins = c(8, 5), row.cex = 0.5) #Create CIM plot with metabolites on the x-axis and treatment x day on the y-axis. 
dev.off()
```
...
plotIndiv(final.splsda.treatmentDay.known, comp = c(1,3), ind.names = FALSE,
          ...
          pch = as.factor(rep(c(1, 2, 0, 4), times = 2)), cex = 3, point.lwd = 0.7,
          ellipse = TRUE, star = TRUE, centroid = FALSE, 
          title = "sPLS-DA by Temperature and Day", X.label = "Component 1: 18% Explained Variance", size.xlabel = 15, Y.label = "Component 3: 9% Explained Variance", size.ylabel = 15, size.axis = 13,
          style = "ggplot2")
#dev.off()
```
...
Based on the PCA, ASCA, and PLS-DAs above, it seems like there is a clear separation between early (days 1 and 7) and late (days 26 and 42) timepoints. I'm going to repeat my analyses using these larger time classifications for the known metabolites.
...
plot(tempTimepoint.ASCAmodel, component = 1:10, type = "scree") #Scree plot of variance explained by each PC. There's a pretty steep drop-off between components 2 and 3. Component 3 may or may not explain more than 50% of the variance.
# dev.off()
```
...
```{r}
tempTimepoint.loadingsPC1 <- get_loadings(tempTimepoint.ASCAmodel, component = 1, effect = 1, n_limit = 10) #Get loadings for the top 10 positively and negatively loaded compounds on the PC
...
```{r}
tempTimepoint.loadingsPC2 <- get_loadings(tempTimepoint.ASCAmodel, component = 2, effect = 1, n_limit = 10) #Get loadings for the top 10 positively and negatively loaded compounds on the PC
...
```{r}
tempTimepoint.loadingsPC3 <- get_loadings(tempTimepoint.ASCAmodel, component = 3, effect = 1, n_limit = 10) #Get loadings for the top 10 positively and negatively loaded compounds on the PC
...
plot(perf.treatmentTimepoint.known.plsda, sd = TRUE) #Overall, increasing the number of components decreases the error rate. There doesn't seem to be an exponential reduction of error rate after 3 components. Centroid distance offers the lowest overall and balanced error rates (BER) when using 3 components
...
```{r}
treatmentTimepoint.known.plsda.tuned <- plsda(X_tempTimepoint, Y_tempTimepoint, ncomp = 3) #Run PLS-DA with the appropriate number of components to include based on tuning above.
...
```{r}
tune.splsda.treatmentTimepoint.known <- tune.splsda(X_tempTimepoint, Y_tempTimepoint, ncomp = 3, 
                                              validation = 'Mfold', folds = 5,
                                              measure = "BER", test.keepX = list.keepX, nrepeat = 50) #More extensive classification performance
...
plotIndiv(final.splsda.treatmentTimepoint.known, comp = c(1,2)) #PC1: 18, PC2: 12
...
plotIndiv(final.splsda.treatmentTimepoint.known, comp = c(1,2), ind.names = FALSE,
          ...
          pch = as.factor(rep(c(1, 2), times = 2)), cex = 3, point.lwd = 0.7,
          ellipse = TRUE, star = TRUE, centroid = FALSE, 
          title = "sPLS-DA by Temperature and Timepoint", X.label = "Component 1: 18% Explained Variance", size.xlabel = 15, Y.label = "Component 2: 12% Explained Variance", size.ylabel = 15, size.axis = 13,
          style = "ggplot2")
#dev.off()
```
...
plotIndiv(final.splsda.treatmentTimepoint.known, comp = c(1,3), ind.names = FALSE,
          ...
          pch = as.factor(rep(c(1, 2), times = 2)), cex = 3, point.lwd = 0.7,
          ellipse = TRUE, star = TRUE, centroid = FALSE, 
          title = "sPLS-DA by Temperature and Timepoint", X.label = "Component 1: 18% Explained Variance", size.xlabel = 15, Y.label = "Component 3: 7% Explained Variance", size.ylabel = 15, size.axis = 13,
          style = "ggplot2")
# dev.off()
```
...
```{r}
treatmentTimepoint.VIP <- as.data.frame(PLSDA.VIP(treatmentTimepoint.known.plsda.tuned)[["tab"]]) %>%
...
head(treatmentTimepoint.VIP) #Confirm changes
```

```{r}
write.csv(treatmentTimepoint.VIP, "PLSDA/known-metabolites-VIP-treatmentTimepoint.csv", quote = FALSE, row.names = FALSE) #Save VIP scores
```

```{r}
treatmentTimepoint.VIP %>%
  filter(VIP >= 1) %>%
  nrow(.) #69 metabolites have VIP > 1
```
...
ggsave("PLSDA/figures/known-metabolites-VIP-treatmentTimepoint.pdf", height = 8.5, width = 11)
```
...
t.test_5v25_early %>%
  filter(., p.adj < 0.05) %>%
  write.csv(., "PLSDA/known-metabolites-significantVIP-5v25-early.csv", row.names = FALSE, quote = FALSE) #Save t-test output for significant VIP
```
...
VIP_select_5v25_late <- cbind(clean_5v25_late[, c(1:2, 199:200)], 
  ...
  pivot_longer(., cols = 5:65, values_to = "log_norm_counts", names_to = "metabolite") %>%
  group_by(., metabolite) #cbind metadata columns (1:4) and log-transformed "clean" data for VIP. Pivot data longer and group by metabolites.
...
t.test_5v25_late %>%
  filter(., p.adj < 0.05) %>%
  write.csv(., "PLSDA/known-metabolites-significantVIP-5v25-late.csv", row.names = FALSE, quote = FALSE) #Save t-test output for significant VIP
```
...
t.test_5v25_late %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>%
  nrow(.) #59 significantly different metabolites
t.test_5v25_late %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>% filter(., estimate1 > estimate2) %>%
  nrow(.) #13 metabolites higher in 5ºC
t.test_5v25_late %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>% filter(., estimate1 < estimate2) %>%
...
VIP_select_5_earlyvlate <- cbind(clean_5_earlyvlate[, c(1:2, 199:200)], 
  ...
  pivot_longer(., cols = 5:54, values_to = "log_norm_counts", names_to = "metabolite") %>%
  group_by(., metabolite) #cbind metadata columns (1:4) and log-transformed "clean" data for VIP. Pivot data longer and group by metabolites.
...
t.test_5_earlyvlate %>%
  filter(., p.adj < 0.05) %>%
  write.csv(., "PLSDA/known-metabolites-significantVIP-5-earlyvlate.csv", row.names = FALSE, quote = FALSE) #Save t-test output for significant VIP
```
...
t.test_5_earlyvlate %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>%
  nrow(.) #44 significantly different metabolites
t.test_5_earlyvlate %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>% filter(., estimate1 > estimate2) %>%
  nrow(.) #38 metabolites higher earlier
t.test_5_earlyvlate %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>% filter(., estimate1 < estimate2) %>%
...
t.test_25_earlyvlate %>%
  filter(., p.adj < 0.05) %>%
  write.csv(., "PLSDA/known-metabolites-significantVIP-25-earlyvlate.csv", row.names = FALSE, quote = FALSE) #Save t-test output for significant VIP
```
...
t.test_25_earlyvlate %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>%
  nrow(.) #47 significantly different metabolites
t.test_25_earlyvlate %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>% filter(., estimate1 > estimate2) %>%
  nrow(.) #43 metabolites higher earlier
t.test_25_earlyvlate %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>% filter(., estimate1 < estimate2) %>%
  nrow(.) #4 metabolites higher later
```
...
```{r}
tempGen.loadingsPC1 <- get_loadings(tempGen.ASCAmodel, component = 1, effect = 1, n_limit = 10) #Get loadings for the top 10 positively and negatively loaded compounds on the PC
...
```{r}
tempGen.loadingsPC2 <- get_loadings(tempGen.ASCAmodel, component = 2, effect = 1, n_limit = 10) #Get loadings for the top 10 positively and negatively loaded compounds on the PC
...
```{r}
tempGen.loadingsPC3 <- get_loadings(tempGen.ASCAmodel, component = 3, effect = 1, n_limit = 10) #Get loadings for the top 10 positively and negatively loaded compounds on the PC
