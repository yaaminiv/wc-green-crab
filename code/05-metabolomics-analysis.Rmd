...
```{r}
# Code drafted with assistance from Gemini

# install.packages("pak") #Modern alternative to devtools that is better at handling dependencies
library(pak)
pak::pkg_install(c(
  "andjar/ALASCA", 
  "url::https://cran.r-project.org/src/contrib/Archive/log4r/log4r_0.4.3.tar.gz"
)) # Install ALASCA and map log4r to the latest cached version on CRAN
library(ALASCA)
```
```{r}
# Fix conflicts in fmt_current_time between different versions of R and ALASCA
# Code drafted with Gemini assistance

fmt_current_time <- function(format, use_utc = FALSE) {
  if (use_utc) {
    format(Sys.time(), format = format, tz = "UTC")
  } else {
    format(Sys.time(), format = format)
  }
} # Define the missing time formatter in your Global Environment
```
...
```{r}
(pcaPlot6 + ggtitle("A. Experimental day")) / (pcaPlot34 + ggtitle("B. Timepoint"))
ggsave("PCA/figures/known-metabolites-PCA-dayTimepoint-multipanel.pdf", height = 11, width = 8.5)
```
...
```{r}
plot(tempTimepoint.ASCAmodel, effect = 1, component = c(1:3), type = 'effect', 
     n_limit = 10, 
     ribbon = TRUE,
     x_angle = 0, x_h_just= 0.5,
     flip_axis = TRUE,
     ...
     x_label = "Timepoint",
     group_label = "Temperature (ºC)")
ggsave("ASCA/figures/tempTimepoint-effect-plot-allPCs.pdf", height = 11, width = 8.5)
```
...
earlyvlate_25_pathwayResults <- read.csv("metaboanalyst/25-earlyvlate_metaboanalyst-results/pathway_results.csv") %>%
  ...
  mutate(., negLogFDR = -log(FDR))
#Import pathway assignment data. Rename metabolite column and create new columns (pop observed hits: number of hits/total metabolites in pathway; enrichment ratio: hits/expected hits)
head(earlyvlate_25_pathwayResults) #Look at pathway analysis results. P-values are from enrichment analysis, impact is from pathway topology analysis
```
...
```{r}
earlyvlate_25_arginineBiosynthesis_facets <- c("1" = "alpha-ketoglutarate",
                                               "2" = "glutamic acid",
                                               "3" = "ornithine",
                                               "4" = "citrulline",
                                               "5" = "aspartic acid",
                                               "6" = "fumaric acid") #Assign labels for facets
```

```{r}
transMetabData_knownTimepoint %>%
  dplyr::select(., c(crab.ID, treatment_timepoint, 
                     `alpha-ketoglutarate`, `glutamic acid`, `ornithine`, `citrulline`, `aspartic acid`, `fumaric acid`)) %>%
  filter(., treatment_timepoint == "25C_early" | treatment_timepoint == "25C_late") %>%
  pivot_longer(., cols = c(3:8), names_to = "metabolite", values_to = "value") %>%
  mutate(value = log(value)) %>% 
  mutate_if(is.numeric, list(~na_if(., -Inf))) %>%
  replace(is.na(.), 0) %>%
  mutate(., order = case_when(metabolite == "alpha-ketoglutarate" ~ 1,
                              metabolite == "glutamic acid" ~ 2,
                              metabolite == "ornithine" ~ 3,
                              metabolite == "citrulline" ~ 4,
                              metabolite == "aspartic acid" ~ 3,
                              metabolite == "fumaric acid" ~ 4)) %>%
  ggplot(., mapping = aes(x = treatment_timepoint, y = value, color = treatment_timepoint)) +
  facet_wrap(~order, scales = "free_y", labeller = labeller(order = earlyvlate_25_arginineBiosynthesis_facets)) +
  ylab("Log-Transformed Abundance") +
  geom_boxplot(width = 0.5, position = position_dodge(width = 0.5), alpha = 0.7) +
  geom_point(pch = 21, position = position_dodge(width = 0.5)) +
  scale_fill_manual(name = "Temperature (ºC) x Timepoint", 
  ...
  theme_classic(base_size = 15) + theme(axis.text.x = element_blank(), 
                                        ...
                                        legend.text = element_text(color = "black", size = 12), 
                                        strip.text.x = element_text(size = 10, color = "black", face="bold"))
ggsave("metaboanalyst/figures/25-earlyvlate_arginine-biosynthesis-plot.pdf", height = 8.5, width = 11)
```
...
Alanine, aspartate, and glutmate metabolism:
...
```{r}
earlyvlate_25_alanineMetabolism_facets <- c("1" = "aspartic acid",
                                            "2" = "asparagine",
                                            "3" = "fumaric acid",
                                            "4" = "alpha-ketoglutarate",
                                            "5" = "glutamic acid") #Assign labels for facets
```

```{r}
transMetabData_knownTimepoint %>%
  dplyr::select(., c(crab.ID, treatment_timepoint, 
                     `aspartic acid`, asparagine, `fumaric acid`, `alpha-ketoglutarate`, `glutamic acid`)) %>%
  filter(., treatment_timepoint == "25C_early" | treatment_timepoint == "25C_late") %>%
  pivot_longer(., cols = c(3:7), names_to = "metabolite", values_to = "value") %>%
  mutate(value = log(value)) %>% 
  mutate_if(is.numeric, list(~na_if(., -Inf))) %>%
  replace(is.na(.), 0) %>%
  mutate(., order = case_when(metabolite == "aspartic acid" ~ 1,
                              metabolite == "asparagine" ~ 2,
                              metabolite == "fumaric acid" ~ 3,
                              metabolite == "alpha-ketoglutarate" ~ 4,
                              metabolite == "glutamic acid" ~ 5)) %>%
  ggplot(., mapping = aes(x = treatment_timepoint, y = value, color = treatment_timepoint)) +
  facet_wrap(~order, scales = "free_y", labeller = labeller(order = earlyvlate_25_alanineMetabolism_facets)) +
  ylab("Log-Transformed Abundance") +
  geom_boxplot(width = 0.5, position = position_dodge(width = 0.5), alpha = 0.7) +
  geom_point(pch = 21, position = position_dodge(width = 0.5)) +
  scale_fill_manual(name = "Temperature (ºC) x Timepoint", 
  ...
  scale_colour_manual(name = "Temperature (ºC) x Timepoint", 
                      ...
                      labels = c("Early 25ºC", 
                                 "Late 25ºC")) +
  xlab("") + #Axis titles
  ggtitle("Alanine, Aspartate, and Glutamate Metabolism") +
  theme_classic(base_size = 15) + theme(axis.text.x = element_blank(), 
                                        axis.ticks.x = element_blank(),
                                        legend.position = "bottom", 
                                        legend.text = element_text(color = "black", size = 12), 
                                        strip.text.x = element_text(size = 10, color = "black", face="bold"))
ggsave("metaboanalyst/figures/earlyvlate-25_alanine-metabolism-plot.pdf", height = 8.5, width = 11)
```
...
```{r}
plot(tempGen.ASCAmodel, effect = 1, component = c(1:3), type = 'effect', 
     n_limit = 10, 
     ribbon = TRUE,
     x_angle = 0, x_h_just= 0.5,
     flip_axis = TRUE,
     ...
     x_label = "Genotype",
     group_label = "Temperature (ºC)")
ggsave("ASCA/figures/tempGen-effect-plot-allPCs.pdf", height = 11, width = 8.5)
```
...
Well, there are no VIP that distinguish genotypes within a temperature. Looking at some of the ASCA trends again, it may be informative to look at how the same genotype may be responding to different temperature conditions.

#### CC: 5ºC vs. 25ºC

```{r}
list <- c("05C_CC", "25C_CC") #Assign treatments to examine
```
...
```{r}
known.plsda_CC_5Cv25C <- plsda(X, Y, ncomp = 4) #Run PLSDA for desired contrast
VIP_CC_5Cv25C <- as.data.frame(PLSDA.VIP(known.plsda_CC_5Cv25C)[["tab"]]) %>%
  rownames_to_column(., var = "metabolite") %>%
  filter(., VIP >= 1) #Extract VIP list and filter for VIP > 1
VIP_CC_5Cv25C
```
...
```{r}
clean_CC_5Cv25C <- transMetabData_knownTimepoint %>%
  dplyr::filter(., treatment_genotype %in% list) %>%
  droplevels() #Filter transMetabData for desired contrasts.
VIP_select_CC_5Cv25C <- cbind(clean_CC_5Cv25C[, c(1:2, 6, 10)], 
                              log(clean_CC_5Cv25C[, names(clean_CC_5Cv25C) %in% VIP_CC_5Cv25C$metabolite])) %>%
  pivot_longer(., cols = 5:63, values_to = "log_norm_counts", names_to = "metabolite") %>%
  group_by(., metabolite) #cbind metadata columns (1:4) and log-transformed "clean" data for VIP. Pivot data longer and group by metabolites.
VIP_select_CC_5Cv25C #Confirm changes
```

```{r}
t.test_CC_5Cv25C <-do(VIP_select_CC_5Cv25C, tidy(t.test(.$log_norm_counts ~ .$treatment_genotype,
                                                        alternative = "two.sided",
                                                        mu = 0,
                                                        var.equal = FALSE,
                                                        conf.level = 0.95
))) #Looped t-test for all metabolites with VIP > 1
t.test_CC_5Cv25C$p.adj <- p.adjust(t.test_CC_5Cv25C$p.value, method = c("fdr"), n = length(VIP_CC_5Cv25C$metabolite)) #Adjust p value for the number of comparisons
t.test_CC_5Cv25C
```
...
t.test_CC_5Cv25C %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>%
  nrow(.) #26 significantly different metabolites
t.test_CC_5Cv25C %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>% filter(., estimate1 > estimate2) %>%
...
t.test_CC_5Cv25C %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>% filter(., estimate1 < estimate2) %>%
  nrow(.) #24 metabolites higher in 25ºC
```
...
t.test_CC_5Cv25C %>%
  ...
  left_join(., (rawMetabolomicsData %>% 
                  dplyr::rename(metabolite = BinBase.name)), 
            by = "metabolite") %>%
  mutate(., higherAbundanceIn = case_when(estimate1 > estimate2 ~ "5C_CC",
                                          estimate1 < estimate2 ~ "25C_CC")) %>%
  dplyr::select(., metabolite, PubChem, KEGG, higherAbundanceIn) %>%
  unique(.) %>%
  write.table("PLSDA/5v25-CC_significantVIP_metabolites.txt", sep = "\t", quote = FALSE, row.names = FALSE) #Take significant pairwise VIP data and join with identifiers. Create a column with information on condition with the higher abundance. Retain unique entries and write table.
```
...
t.test_CC_5Cv25C %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>%
  left_join(., VIP_CC_5Cv25C, by = "metabolite") %>%
  mutate(., direction = case_when(estimate > 0 ~ "5",
                                  estimate < 0 ~ "25")) %>%
  mutate(., score = case_when(estimate > 0 ~ VIP,
                              estimate < 0 ~ -VIP)) %>%
  ...
  scale_fill_manual(values = c(plotColors[3], plotColors[1]), 
                    ...
                    labels = c("5", "25")) +
  theme_classic(base_size = 15) + theme() #Take t-test results and filter out significant metabolites. Create two new columns using estimate values: one for the direction of increase, and another for positive or negative VIP. Arrange by metabolite name and reorder by cumulative VIP score. Plot bars and change x and y axis labels. Add scale information.
ggsave("PLSDA/figures/significant-VIP-5v25-CC.pdf", height = 8.5, width = 11)
```

#### CT: 5ºC vs. 25ºC

```{r}
list <- c("05C_CT", "25C_CT") #Assign treatments to examine
```
...
```{r}
known.plsda_CT_5Cv25C <- plsda(X, Y, ncomp = 4) #Run PLSDA for desired contrast
VIP_CT_5Cv25C <- as.data.frame(PLSDA.VIP(known.plsda_CT_5Cv25C)[["tab"]]) %>%
  rownames_to_column(., var = "metabolite") %>%
  filter(., VIP >= 1) #Extract VIP list and filter for VIP > 1
VIP_CT_5Cv25C
```
...
```{r}
clean_CT_5Cv25C <- transMetabData_knownTimepoint %>%
  dplyr::filter(., treatment_genotype %in% list) %>%
  droplevels() #Filter transMetabData for desired contrasts.
VIP_select_CT_5Cv25C <- cbind(clean_CT_5Cv25C[, c(1:2, 6, 10)], 
                              log(clean_CT_5Cv25C[, names(clean_CT_5Cv25C) %in% VIP_CT_5Cv25C$metabolite])) %>%
  pivot_longer(., cols = 5:73, values_to = "log_norm_counts", names_to = "metabolite") %>%
  group_by(., metabolite) #cbind metadata columns (1:4) and log-transformed "clean" data for VIP. Pivot data longer and group by metabolites.
VIP_select_CT_5Cv25C #Confirm changes
```

```{r}
t.test_CT_5Cv25C <-do(VIP_select_CT_5Cv25C, tidy(t.test(.$log_norm_counts ~ .$treatment_genotype,
                                                        alternative = "two.sided",
                                                        mu = 0,
                                                        var.equal = FALSE,
                                                        conf.level = 0.95
))) #Looped t-test for all metabolites with VIP > 1
t.test_CT_5Cv25C$p.adj <- p.adjust(t.test_CT_5Cv25C$p.value, method = c("fdr"), n = length(VIP_CT_5Cv25C$metabolite)) #Adjust p value for the number of comparisons
t.test_CT_5Cv25C
```
...
t.test_CT_5Cv25C %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>%
  nrow(.) #35 significantly different metabolites
t.test_CT_5Cv25C %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>% filter(., estimate1 > estimate2) %>%
  nrow(.) #6 metabolites higher in 5ºC
t.test_CT_5Cv25C %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>% filter(., estimate1 < estimate2) %>%
  nrow(.) #29 metabolites higher in 25ºC
```
...
t.test_CT_5Cv25C %>%
  ...
  left_join(., (rawMetabolomicsData %>% 
                  dplyr::rename(metabolite = BinBase.name)), 
            by = "metabolite") %>%
  mutate(., higherAbundanceIn = case_when(estimate1 > estimate2 ~ "5C_CT",
                                          estimate1 < estimate2 ~ "25C_CT")) %>%
  dplyr::select(., metabolite, PubChem, KEGG, higherAbundanceIn) %>%
  unique(.) %>%
  write.table("PLSDA/5v25-CT_significantVIP_metabolites.txt", sep = "\t", quote = FALSE, row.names = FALSE) #Take significant pairwise VIP data and join with identifiers. Create a column with information on condition with the higher abundance. Retain unique entries and write table.
```
...
t.test_CT_5Cv25C %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>%
  left_join(., VIP_CT_5Cv25C, by = "metabolite") %>%
  mutate(., direction = case_when(estimate > 0 ~ "5",
                                  estimate < 0 ~ "25")) %>%
  mutate(., score = case_when(estimate > 0 ~ VIP,
                              estimate < 0 ~ -VIP)) %>%
  ...
  scale_fill_manual(values = c(plotColors[3], plotColors[1]), 
                    ...
                    labels = c("5", "25")) +
  theme_classic(base_size = 15) + theme() #Take t-test results and filter out significant metabolites. Create two new columns using estimate values: one for the direction of increase, and another for positive or negative VIP. Arrange by metabolite name and reorder by cumulative VIP score. Plot bars and change x and y axis labels. Add scale information.
ggsave("PLSDA/figures/significant-VIP-5v25-CT.pdf", height = 8.5, width = 11)
```

#### TT: 5ºC vs. 25ºC

```{r}
list <- c("05C_TT", "25C_TT") #Assign treatments to examine
```
...
```{r}
known.plsda_TT_5Cv25C <- plsda(X, Y, ncomp = 4) #Run PLSDA for desired contrast
VIP_TT_5Cv25C <- as.data.frame(PLSDA.VIP(known.plsda_TT_5Cv25C)[["tab"]]) %>%
  rownames_to_column(., var = "metabolite") %>%
  filter(., VIP >= 1) #Extract VIP list and filter for VIP > 1
VIP_TT_5Cv25C
```
...
```{r}
clean_TT_5Cv25C <- transMetabData_knownTimepoint %>%
  dplyr::filter(., treatment_genotype %in% list) %>%
  droplevels() #Filter transMetabData for desired contrasts.
VIP_select_TT_5Cv25C <- cbind(clean_TT_5Cv25C[, c(1:2, 6, 10)], 
                              log(clean_TT_5Cv25C[, names(clean_TT_5Cv25C) %in% VIP_TT_5Cv25C$metabolite])) %>%
  pivot_longer(., cols = 5:75, values_to = "log_norm_counts", names_to = "metabolite") %>%
  group_by(., metabolite) #cbind metadata columns (1:4) and log-transformed "clean" data for VIP. Pivot data longer and group by metabolites.
VIP_select_TT_5Cv25C #Confirm changes
```

```{r}
t.test_TT_5Cv25C <-do(VIP_select_TT_5Cv25C, tidy(t.test(.$log_norm_counts ~ .$treatment_genotype,
                                                        alternative = "two.sided",
                                                        mu = 0,
                                                        var.equal = FALSE,
                                                        conf.level = 0.95
))) #Looped t-test for all metabolites with VIP > 1
t.test_TT_5Cv25C$p.adj <- p.adjust(t.test_TT_5Cv25C$p.value, method = c("fdr"), n = length(VIP_TT_5Cv25C$metabolite)) #Adjust p value for the number of comparisons
t.test_TT_5Cv25C
```
...
t.test_TT_5Cv25C %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>% filter(., estimate1 > estimate2) %>%
  nrow(.) #3 metabolites higher in 5ºC
t.test_TT_5Cv25C %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>% filter(., estimate1 < estimate2) %>%
  nrow(.) #26 metabolites higher in 25ºC
```
...
t.test_TT_5Cv25C %>%
  ...
  left_join(., (rawMetabolomicsData %>% 
                  dplyr::rename(metabolite = BinBase.name)), 
            by = "metabolite") %>%
  mutate(., higherAbundanceIn = case_when(estimate1 > estimate2 ~ "5C_TT",
                                          estimate1 < estimate2 ~ "25C_TT")) %>%
  dplyr::select(., metabolite, PubChem, KEGG, higherAbundanceIn) %>%
  unique(.) %>%
  write.table("PLSDA/5v25-TT_significantVIP_metabolites.txt", sep = "\t", quote = FALSE, row.names = FALSE) #Take significant pairwise VIP data and join with identifiers. Create a column with information on condition with the higher abundance. Retain unique entries and write table.
```
...
t.test_TT_5Cv25C %>%
  ungroup(.) %>%
  filter(., p.adj < 0.05) %>%
  left_join(., VIP_TT_5Cv25C, by = "metabolite") %>%
  mutate(., direction = case_when(estimate > 0 ~ "5",
                                  estimate < 0 ~ "25")) %>%
  mutate(., score = case_when(estimate > 0 ~ VIP,
                              estimate < 0 ~ -VIP)) %>%
  ...
  scale_fill_manual(values = c(plotColors[3], plotColors[1]), 
                    ...
                    labels = c("5", "25")) +
  theme_classic(base_size = 15) + theme() #Take t-test results and filter out significant metabolites. Create two new columns using estimate values: one for the direction of increase, and another for positive or negative VIP. Arrange by metabolite name and reorder by cumulative VIP score. Plot bars and change x and y axis labels. Add scale information.
ggsave("PLSDA/figures/significant-VIP-5v25-TT.pdf", height = 8.5, width = 11)
```

#### Unique VIP for each genotype-temperature contrast

Above, I identified VIP that drove differences between temperatures for each genotype. I want to know if there are any VIP that are unique for a specific genotype.

```{r}
anti_join(x = t.test_CC_5Cv25C %>% filter(., p.adj < 0.05) %>% dplyr::select(metabolite),
          y = t.test_CT_5Cv25C %>% filter(., p.adj < 0.05) %>% dplyr::select(metabolite),
          by = "metabolite") %>%
  anti_join(x = .,
            y = t.test_TT_5Cv25C %>% filter(., p.adj < 0.05) %>% dplyr::select(metabolite),
            by = "metabolite") %>%
  nrow(.) #9 metabolites unique to this comparison
```
...
anti_join(x = t.test_CC_5Cv25C %>% filter(., p.adj < 0.05) %>% dplyr::select(metabolite),
  ...
  anti_join(x = .,
            y = t.test_TT_5Cv25C %>% filter(., p.adj < 0.05) %>% dplyr::select(metabolite),
            by = "metabolite") %>%
  right_join(x = t.test_CC_5Cv25C, y = ., by = "metabolite") %>%
  filter(., p.adj < 0.05) %>%
  left_join(., (rawMetabolomicsData %>% 
                  dplyr::rename(metabolite = BinBase.name)), 
            by = "metabolite") %>%
  ...
  dplyr::select(., metabolite, PubChem, KEGG, higherAbundanceIn) %>%
  unique(.) %>%
  write.table("PLSDA/5v25-CC_significantVIP_metabolitesUnique.txt", sep = "\t", quote = FALSE, row.names = FALSE)
#Use a series of anti-joins to identify metabolites that are unique VIP for driving temperature differences in CC crabs. Match it back with t-test results. Retain unique entries and save table.
```

```{r}
anti_join(x = t.test_CT_5Cv25C %>% filter(., p.adj < 0.05) %>% dplyr::select(metabolite),
          y = t.test_CC_5Cv25C %>% filter(., p.adj < 0.05) %>% dplyr::select(metabolite),
          by = "metabolite") %>%
  anti_join(x = .,
            y = t.test_TT_5Cv25C %>% filter(., p.adj < 0.05) %>% dplyr::select(metabolite),
            by = "metabolite") %>%
  nrow(.) #14 metabolites unique to this comparison
```
...
anti_join(x = t.test_CT_5Cv25C %>% filter(., p.adj < 0.05) %>% dplyr::select(metabolite),
  ...
  anti_join(x = .,
            y = t.test_TT_5Cv25C %>% filter(., p.adj < 0.05) %>% dplyr::select(metabolite),
            by = "metabolite") %>%
  right_join(x = t.test_CT_5Cv25C, y = ., by = "metabolite") %>%
  filter(., p.adj < 0.05) %>%
  left_join(., (rawMetabolomicsData %>% 
                  dplyr::rename(metabolite = BinBase.name)), 
            by = "metabolite") %>%
  ...
  dplyr::select(., metabolite, PubChem, KEGG, higherAbundanceIn) %>%
  unique(.) %>%
  write.table("PLSDA/5v25-CT_significantVIP_metabolitesUnique.txt", sep = "\t", quote = FALSE, row.names = FALSE)
#Use a series of anti-joins to identify metabolites that are unique VIP for driving temperature differences in CT crabs. Match it back with t-test results. Retain unique entries and save table.
```

```{r}
anti_join(x = t.test_TT_5Cv25C %>% filter(., p.adj < 0.05) %>% dplyr::select(metabolite),
          y = t.test_CC_5Cv25C %>% filter(., p.adj < 0.05) %>% dplyr::select(metabolite),
          by = "metabolite") %>%
  anti_join(x = .,
            y = t.test_CT_5Cv25C %>% filter(., p.adj < 0.05) %>% dplyr::select(metabolite),
            by = "metabolite") %>%
  nrow(.) #11 metabolites unique to this comparison
```
...
anti_join(x = t.test_TT_5Cv25C %>% filter(., p.adj < 0.05) %>% dplyr::select(metabolite),
  ...
  anti_join(x = .,
            y = t.test_CT_5Cv25C %>% filter(., p.adj < 0.05) %>% dplyr::select(metabolite),
            by = "metabolite") %>%
  right_join(x = t.test_TT_5Cv25C, y = ., by = "metabolite") %>%
  filter(., p.adj < 0.05) %>%
  left_join(., (rawMetabolomicsData %>% 
                  dplyr::rename(metabolite = BinBase.name)), 
            by = "metabolite") %>%
  ...
  dplyr::select(., metabolite, PubChem, KEGG, higherAbundanceIn) %>%
  unique(.) %>%
  write.table("PLSDA/5v25-TT_significantVIP_metabolitesUnique.txt", sep = "\t", quote = FALSE, row.names = FALSE)
#Use a series of anti-joins to identify metabolites that are unique VIP for driving temperature differences in CC crabs. Match it back with t-test results. Retain unique entries and save table.
```
...
```{r}
CC_5v25_pathwayResults <- read.csv("metaboanalyst/CC-5v25_metaboanalyst-results/pathway_results.csv") %>%
  dplyr::rename(metabolite = "X") %>%
  mutate(., Observed = Hits/Total) %>%
  mutate(., Significant = case_when(FDR < 0.05 ~ "Yes",
                                    FDR >= 0.05 ~ "No")) %>%
  ...
  mutate(., negLogFDR = -log(FDR))
#Import pathway assignment data. Rename metabolite column and create new columns (pop observed hits: number of hits/total metabolites in pathway; enrichment ratio: hits/expected hits)
head(CC_5v25_pathwayResults) #Look at pathway analysis results. P-values are from enrichment analysis, impact is from pathway topology analysis
```

```{r}
CC_5v25_pathwayResults %>%
  ggplot(., aes(x = Impact, y = X.log10.p., size = enrichmentRatio, color = Significant)) +
  geom_point(alpha = 0.6) +
  labs(x = "Pathway Impact", y = "Scaled Enrichment P-Value") +
  ylim(0, 6) +
  ...
  scale_color_manual(name = "FDR",
                     ...
                     labels = c(expression("">=0.05), expression(""<0.05))) +
  theme_classic(base_size = 15)
ggsave("metaboanalyst/figures/CC_5v25-pathwaySignificance.pdf", heigh = 8.5, width = 11)
```
Glutathione metabolism:
...
```{r}
CC_5v25_glutathioneMetabolism_facets <- c("1" = "glycine",
                                          "2" = "glutathione",
                                          "3" = "oxoproline",
                                          "4" = "cysteine") #Assign labels for facets
```

```{r}
transMetabData %>%
  filter(., treatment != "15C") %>%
  filter(., final.genotype == "CC") %>%
  dplyr::select(., c(crab.ID, treatment, 
                     glycine, glutathione, oxoproline, cysteine)) %>%
  pivot_longer(., cols = c(3:6), names_to = "metabolite", values_to = "value") %>%
  mutate(value = log(value)) %>% 
  mutate_if(is.numeric, list(~na_if(., -Inf))) %>%
  replace(is.na(.), 0) %>%
  mutate(., order = case_when(metabolite == "glycine" ~ 1,
                              metabolite == "glutathione" ~ 2,
                              metabolite == "oxoproline" ~ 3,
                              metabolite == "cysteine" ~ 4)) %>%
  ggplot(., mapping = aes(x = treatment, y = value, color = treatment)) +
  facet_wrap(~order, scales = "free_y", labeller = labeller(order = CC_5v25_glutathioneMetabolism_facets)) +
  ylab("Log-Transformed Abundance") +
  geom_boxplot(width = 0.5, position = position_dodge(width = 0.5), alpha = 0.7) +
  geom_point(pch = 21, position = position_dodge(width = 0.5)) +
  ...
  theme_classic(base_size = 15) + theme(axis.text.x = element_blank(), 
                                        ...
                                        legend.text = element_text(color = "black", size = 12), 
                                        strip.text.x = element_text(size = 10, color = "black", face="bold"))
ggsave("metaboanalyst/figures/CC-5v25_glutathione-metabolism-plot.pdf", height = 8.5, width = 11)
```

##### CT: 5ºC vs. 25ºC

```{r}
CT_5v25_pathwayResults <- read.csv("metaboanalyst/CT-5v25_metaboanalyst-results/pathway_results.csv") %>%
  dplyr::rename(metabolite = "X") %>%
  mutate(., Observed = Hits/Total) %>%
  mutate(., Significant = case_when(FDR < 0.05 ~ "Yes",
                                    FDR >= 0.05 ~ "No")) %>%
  ...
  mutate(., negLogFDR = -log(FDR))
#Import pathway assignment data. Rename metabolite column and create new columns (pop observed hits: number of hits/total metabolites in pathway; enrichment ratio: hits/expected hits)
head(CT_5v25_pathwayResults) #Look at pathway analysis results. P-values are from enrichment analysis, impact is from pathway topology analysis
```

```{r}
CT_5v25_pathwayResults %>%
  ggplot(., aes(x = Impact, y = X.log10.p., size = enrichmentRatio, color = Significant)) +
  geom_point(alpha = 0.6) +
  labs(x = "Pathway Impact", y = "Scaled Enrichment P-Value") +
  ylim(0, 6) +
  ...
  scale_color_manual(name = "FDR",
                     ...
                     labels = c(expression("">=0.05), expression(""<0.05))) +
  theme_classic(base_size = 15)
ggsave("metaboanalyst/figures/CT_5v25-pathwaySignificance.pdf", heigh = 8.5, width = 11)
```

No significant pathways, will not pursue additional visualization.

##### TT: 5ºC vs. 25ºC

```{r}
TT_5v25_pathwayResults <- read.csv("metaboanalyst/TT-5v25_metaboanalyst-results/pathway_results.csv") %>%
  dplyr::rename(metabolite = "X") %>%
  mutate(., Observed = Hits/Total) %>%
  mutate(., Significant = case_when(FDR < 0.05 ~ "Yes",
                                    FDR >= 0.05 ~ "No")) %>%
  ...
  mutate(., negLogFDR = -log(FDR))
#Import pathway assignment data. Rename metabolite column and create new columns (pop observed hits: number of hits/total metabolites in pathway; enrichment ratio: hits/expecTed hits)
head(TT_5v25_pathwayResults) #Look at pathway analysis results. P-values are from enrichment analysis, impaTT is from pathway topology analysis
```

```{r}
TT_5v25_pathwayResults %>%
  ggplot(., aes(x = Impact, y = X.log10.p., size = enrichmentRatio, color = Significant)) +
  geom_point(alpha = 0.6) +
  labs(x = "Pathway Impact", y = "Scaled Enrichment P-Value") +
  ylim(0, 6) +
  ...
  scale_color_manual(name = "FDR",
                     ...
                     labels = c(expression("">=0.05), expression(""<0.05))) +
  theme_classic(base_size = 15)
ggsave("metaboanalyst/figures/TT_5v25-pathwaySignificance.pdf", heigh = 8.5, width = 11)
```
...
```{r}
CC_5v25_unique_pathwayResults <- read.csv("metaboanalyst/CC-5v25-unique_metaboanalyst-results/pathway_results.csv") %>%
  dplyr::rename(metabolite = "X") %>%
  mutate(., Observed = Hits/Total) %>%
  mutate(., Significant = case_when(FDR < 0.05 ~ "Yes",
                                    FDR >= 0.05 ~ "No")) %>%
  ...
  mutate(., negLogFDR = -log(FDR))
#Import pathway assignment data. Rename metabolite column and create new columns (pop observed hits: number of hits/total metabolites in pathway; enrichment ratio: hits/expected hits)
head(CC_5v25_unique_pathwayResults) #Look at pathway analysis results. P-values are from enrichment analysis, impact is from pathway topology analysis
```

```{r}
CC_5v25_unique_pathwayResults %>%
  ggplot(., aes(x = Impact, y = X.log10.p., size = enrichmentRatio, color = Significant)) +
  geom_point(alpha = 0.6) +
  labs(x = "Pathway Impact", y = "Scaled Enrichment P-Value") +
  ylim(0, 6) +
  ...
  scale_color_manual(name = "FDR",
                     ...
                     labels = c(expression("">=0.05), expression(""<0.05))) +
  theme_classic(base_size = 15)
ggsave("metaboanalyst/figures/CC_5v25_unique-pathwaySignificance.pdf", heigh = 8.5, width = 11)
```
...
```{r}
CT_5v25_unique_pathwayResults <- read.csv("metaboanalyst/CT-5v25-unique_metaboanalyst-results/pathway_results.csv") %>%
  dplyr::rename(metabolite = "X") %>%
  mutate(., Observed = Hits/Total) %>%
  mutate(., Significant = case_when(FDR < 0.05 ~ "Yes",
                                    FDR >= 0.05 ~ "No")) %>%
  ...
  mutate(., negLogFDR = -log(FDR))
#Import pathway assignment data. Rename metabolite column and create new columns (pop observed hits: number of hits/total metabolites in pathway; enrichment ratio: hits/expected hits)
head(CT_5v25_unique_pathwayResults) #Look at pathway analysis results. P-values are from enrichment analysis, impact is from pathway topology analysis
```

```{r}
CT_5v25_unique_pathwayResults %>%
  ggplot(., aes(x = Impact, y = X.log10.p., size = enrichmentRatio, color = Significant)) +
  geom_point(alpha = 0.6) +
  labs(x = "Pathway Impact", y = "Scaled Enrichment P-Value") +
  ylim(0, 6) +
  ...
  scale_color_manual(name = "FDR",
                     ...
                     labels = c(expression("">=0.05), expression(""<0.05))) +
  theme_classic(base_size = 15)
ggsave("metaboanalyst/figures/CT_5v25_unique-pathwaySignificance.pdf", heigh = 8.5, width = 11)
```
...
```{r}
TT_5v25_unique_pathwayResults <- read.csv("metaboanalyst/TT-5v25-unique_metaboanalyst-results/pathway_results.csv") %>%
  dplyr::rename(metabolite = "X") %>%
  mutate(., Observed = Hits/Total) %>%
  mutate(., Significant = case_when(FDR < 0.05 ~ "Yes",
                                    FDR >= 0.05 ~ "No")) %>%
  ...
  mutate(., negLogFDR = -log(FDR))
#Import pathway assignment data. Rename metabolite column and create new columns (pop observed hits: number of hits/total metabolites in pathway; enrichment ratio: hits/expected hits)
head(TT_5v25_unique_pathwayResults) #Look at pathway analysis results. P-values are from enrichment analysis, impact is from pathway topology analysis
```

```{r}
TT_5v25_unique_pathwayResults %>%
  ggplot(., aes(x = Impact, y = X.log10.p., size = enrichmentRatio, color = Significant)) +
  geom_point(alpha = 0.6) +
  labs(x = "Pathway Impact", y = "Scaled Enrichment P-Value") +
  ylim(0, 6) +
  ...
  scale_color_manual(name = "FDR",
                     ...
                     labels = c(expression("">=0.05), expression(""<0.05))) +
  theme_classic(base_size = 15)
ggsave("metaboanalyst/figures/TT_5v25_unique-pathwaySignificance.pdf", heigh = 8.5, width = 11)
```
No significant pathways, will not pursue additional visualization.
...
# Supergene annotations

```{r}
read.delim("../../metadata/RNA-Seq/Cmaenas_inversion_contig_annotations.tsv", sep = "\t", skip = 4)
```
